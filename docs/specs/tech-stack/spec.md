...
- TypeScript strict. 린트·포맷은 Biome + Ultracite 프리셋(`biome.jsonc`에서
  `extends: ["ultracite/biome/core", "ultracite/biome/react"]`). 에이전트
  규칙 파일·편집 후 자동 fix 훅까지 `ultracite init`으로 생성한다. 규칙이
  과하다고 판단되면 extends를 빼서 plain Biome으로 복귀(전환 비용 거의 없음).
...
- **mobile — jest-expo + React Native Testing Library**: bun test가 RN
  변환을 지원하지 않아 모바일만 Jest를 쓴다(버전은 jest-expo가 정한다 —
  "확인된 것" 참조). 러너 2개 공존은 turbo 태스크(`turbo run test`) 뒤로
  감춘다.
...
셋업(태스크 01)에서 실물로 확인해 미확인 가정을 대체한 항목.

- **Uniwind 무료로 충분하다.** 무료판은 MIT이고 프로젝트 수 제한이 없으며
  Tailwind v4 전체 바인딩·라이트/다크/커스텀 테마·플랫폼 셀렉터·RN 컴포넌트
  className 자동 매핑·Expo Go 지원을 모두 포함한다. Pro가 더하는 것은 C++
  엔진(제로 리렌더 ShadowTree), className 기반 Reanimated 4 애니메이션,
  네이티브 테마 전환, `group-active:*`/`group-focus:*` 그룹 변형, 네이티브
  세이프에어리어 인셋, 우선 지원이다. 우리는 Uniwind를 레이아웃·간격·타이포
  유틸리티로 쓰고 애니메이션은 Reanimated를 직접 부르므로 무료 범위 밖으로
  나갈 일이 없다. 또한 Pro는 dev client가 필수라 Expo Go를 쓰는 현재 루프와
  맞지 않는다. 리스트가 길어져 리렌더 병목이 실측되면 그때 재검토한다.
- **모바일 테스트 러너는 Jest 30이 아니라 Jest 29다.** jest-expo(SDK 57)가
  jest 29 계열에 의존한다(`expo install jest`가 29.7을 고른다). 러너 2개
  공존이라는 결정은 그대로고 버전만 다르다.
- **bun 링커는 hoisted로 고정.** bun의 isolated 설치에서는 React Native
  툴체인(`babel-preset-expo`, `@react-native/*`)이 해석되지 않는다. 루트
  `bunfig.toml`에 `linker = "hoisted"`.
...
- **Uniwind 성숙도**: 2025년 출시된 신생 라이브러리. 문제가 생기면 같은
  className 모델인 NativeWind로 전환 경로가 있다. 유료 범위는 확인됐다
  (아래 "확인된 것" 참조) — 무료로 충분하다.
