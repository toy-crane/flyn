...
- [ ] 시뮬레이터에서 앱이 실행되고 Uniwind 스타일이 적용된다
- [ ] Apple 또는 Google 로그인으로 세션을 얻는다 (시뮬레이터/실기기)
- [ ] RLS: 본인 행 CRUD 성공, 타인 행 접근 거부가 pgTAP 테스트로 증명된다
- [ ] Hono 엔드포인트가 유효/무효 JWT를 옳게 통과/거부한다 (테스트 포함)
- [ ] 앱 화면에서 AI 응답이 토큰 단위로 스트리밍 렌더된다 (로컬·배포 모두)
- [ ] `turbo run test`·`lint` 한 명령으로 전 워크스페이스 검사가 돌고 CI가
      PR마다 실행한다
