...
상태: 구현 완료. 사용자가 2026-09-09에 승인한 방향으로 구현하고,
iOS 시뮬레이터와 Android 에뮬레이터에서 확인했다.
...
2026-09-09에 상세와 두 기록 경로를 Root Stack으로 옮겼다. 기존 URL과
화면 본문은 유지하고, 주소에 따른 `NativeTabs.hidden` 변경을 제거했다.

- 실제 Expo Router를 사용하는 테스트로 세 진입 경로와 다음 화 이후 복귀를
  확인했다. 탐색에서 공통 기록을 열고 닫으면 스토리 탭으로 이동하던 테스트가
  수정 전 실패하고 수정 후 통과했다.
- 모바일 전체 72개 테스트 묶음, 510개 테스트와 루트 `check`, `check-types`를
  통과했다. 전체 구현 변경을 명세와 모바일 UI 결정 계약에 맞춰 리뷰했고,
  수정이 필요한 지적은 없었다.
- slot 1의 `flyn-slot-1`과 `flyn_dev_1`에서 표의 세 경로, 새 대화,
  진행 중인 회차, 끝난 화 읽기 전용, 다음 화 이동, AI 질문 시트와 복귀를 확인했다.
  실제 메시지 전송, 표현 확인과 기록 갱신도 확인했다.
- iOS 뒤로 가기 제스처 완료·취소와 Android 헤더·시스템 뒤로 가기를 확인했다.
  Android의 짧은 가장자리 제스처 뒤에도 출발 화면에 머물렀다.
  밝은 화면과 어두운 화면의 전환 녹화에서 탭 없는 화면끼리 이동할 때 탭이
  나타나지 않았고, 목록으로 돌아온 뒤 세 탭을 사용할 수 있었다.
- 입력 없이 닫은 대화는 기록을 추가하지 않았다. 빈 기록과 API 응답을 잠시
  멈춰 만든 조회 중·조회 실패 화면에서도 헤더와 뒤로 가기를 확인했다.
  헤더 중복, 하단 입력창·시작 버튼 가림과 관련 Metro 오류는 발견하지 못했다.

검증은 위 두 가상 기기의 개발 빌드에서 진행했다. 로컬 이메일 코드로 로그인하고
검증 후 로그아웃했다. API는 복구했으며 slot 1의 개발 서버를 유지한다.
검증 중 발견한 별개 로컬 환경 문제는
[이메일 템플릿 후속 작업](../../follow-ups/local-email-template-lost-after-worktree-removal.md)에 남겼다.
