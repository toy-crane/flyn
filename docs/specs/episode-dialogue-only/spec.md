...
- 데이터베이스의 화당 등장인물 상한을 지금의 20명에서 3명으로 내린다. 이 변경은 스키마 변경이며 이 스펙이 그 범위를 정한다. 기존 각본은 모두 상한 안에 있다.
...
- 화에 등장인물 이름을 넷 이상 넣으면 데이터베이스가 거절한다. 기존 각본은 모두 그대로 들어간다.
...
- 인물 색은 스토리 안 등장 순서로 정한다. 그 스토리의 모든 화를 순서대로 훑어 처음 나온 인물부터 1번, 2번 색을 받는다. 같은 인물은 어느 화에서나 같은 번호이고 같은 스토리의 두 인물은 항상 다른 번호다. 번호는 화를 여는 응답이 함께 나르며 앱은 받은 번호를 색으로 바꿔 그린다. 별도 색 필드와 인물 테이블을 두지 않는다. 한 화의 등장인물 순서로 정하지 않는 이유는 지금 각본에서 같은 인물이 화마다 다른 순서로 나오기 때문이다. 앞 화에 새 인물을 넣으면 뒤 인물의 번호가 밀리는데, 실제 사용자가 없는 지금은 감수한다.
...
- 인물 테이블은 만들지 않는다. 인물 초상을 붙이는 단위에서 이름, 색, 초상을 함께 옮길 자리로 검토한다.
