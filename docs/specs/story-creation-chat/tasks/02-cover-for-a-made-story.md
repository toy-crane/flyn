...
- Verification: 마지막 전체 검사를 한 번에 돌렸다. `bun run check`,
  `bun run check-types`, `bun run test`가 지나가고, CI와 같은 격리 DB에서
  마이그레이션, lint, pgTAP, 타입, 스키마 diff와 `20260910071055`의 데이터
  보존 검사가 모두 지나간다. 통합 검사는 25개가 지나가고 1개를 건너뛴다.
  공유 로컬 스택에는 기기 검증이 남긴 행이 있어 거기서 `db:test`를 그대로
  돌리면 실패한다. 다른 slot이 그 스택을 쓰고 있어 되돌리지 않고 격리 검증을
  썼다. iOS에서 `병원 예약 확인`을 새로 만들어 만드는 중 표시, 1화, 교정,
  표현 저장, 결말 카드, 표현 돌아보기, 2화 예고까지 `PRODUCT.md`의 핵심
  흐름을 지났고, 같은 표지가 탐색, 상세, 대화 기록, 스토리 탭 네 곳에
  보였다. 표현 확인이 한 번 실패했고 다시 시도로 통과했다.
- Blocker: —
- Revision: 표지 그림의 장소는 카드가 든다. 표지는 각본과 나란히 시작하므로
  ...
  RLS가 판정하는 것이 요지라서 진짜 로그인이 필요하고, 로그인 헬퍼는 로컬
  스택의 주소를 읽는다. 임시 스택 방식은 secret key로 붙는 기존 표지 검사에
  맞춘 것이다. 같은 폴더의 다른 통합 검사도 로컬 스택을 쓴다.
