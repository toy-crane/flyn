...
- 플린이 충분히 알았다고 판단하면 정리 문장을 따로 쓰지 않고 스토리 카드를
  내놓는다. 카드는 제목, 한 줄 소개, 인물(이름과 역할 한 줄), 화별 제목과
  상황 설명 한 줄을 보여 주고, 아래에 Primary `대화 시작하기`를 둔다. 상황 설명은
  스토리 상세의 에피소드 목록에 보이는 그 문장이다.
- 인물은 스토리에 4명까지이고, 한 화에는 그중 1명에서 3명이 나온다. 2명이 기본이며
  3명은 압박을 더하는 자리에만 쓴다. 카드에 적힌 인물 순서가 스토리 안 순서이고, 이
  순서가 에피소드 화면의 이름표 색이 된다.
...
- 같은 순간에 표지 그림도 한 장 만든다. 각본과 나란히 만들어 기다림을 늘리지 않고,
  스토리를 저장할 때 표지 경로와 BlurHash를 함께 저장한다. 표지가 각본보다 늦으면
  1화는 그대로 열고, 표지는 준비되는 대로 탐색과 상세에 나타난다. 표지 생성이
...
- 만든 스토리는 탐색의 `전체`와 `내 스토리`에 만든 순서의 역순으로 보인다. 표지는
  만들 때 생성한 그림이고, 공식 스토리와 같은 표지 표시 규칙(빈 색 상자 → BlurHash
  → 원본)을 따른다.
...
- 화마다 상황 줄 문구와 이모지, 도입(장면 서술과 첫 대사), 무대, 그 화에 나오는
  인물, 성공·타협·실패 결말 기준을 갖춘다. 화의 제목과 상황 설명은 카드의 개요에서
  그대로 온다. 스토리에는 소개 문단과 완주 안내가 있다.
- 인물은 스토리 단위로 한 번 쓴다. 이름, 스토리 안 순서, 화가 바뀌어도 같은 설명을
  갖고, 설명에는 나이와 직업, 말투, 무엇에 어떻게 반응하는지가 들어간다. 그 화에서의
  사정은 무대의 상황 항목에 쓴다. 무대에 등장인물 문장과 인물 설명을 쓰지 않는다.
  그것은 인물 데이터에서 만든다.
- 도입은 한국어 장면 서술 한 덩어리로 시작한다. 장소와 시각, 첫 대사 전에 벌어진
  일, 사용자가 가진 것과 아는 것을 세 문장 이하로 쓰고 한 문장은 한 줄이다. 함께
  있는 사람과 지난 화의 연결은 첫 대사가 맡는다. 도입 뒤에는 영어 인물 대사만
  이어지고 지문은 없다. 대사는 줄 머리에 `이름:`을 붙이고, 그 이름은 그 화에 나오는
  인물이어야 한다. 대사 안에서 자기 행동을 서술하지 않는다.
- 도입은 이미 문제가 벌어진 자리에서 시작하고, 사용자가 말을 해야 풀린다.
- 사건은 분기하지 않는다. 각 화의 사건은 앞 화가 어느 결말로 끝났든 성립해야 한다.
- 모든 글은 공식 콘텐츠의 길이 한도 안에 든다.

### 표지가 지켜야 할 것

- 공식 표지와 같은 모양이다. 단색 배경에 상대 인물 한 명의 상반신 일러스트이고, 글자,
  로고, 배경 풍경은 넣지 않는다. 스타일을 말하는 문구는 제품이 고정하고, 스토리마다
  바뀌는 것은 인물 설명과 장소뿐이다.
- 그림에 쓰는 인물은 카드의 인물 중 순서 1번이다. 사용자가 적은 실제 사람이나 회사
  이름은 그림 문구에 넣지 않고 인물의 역할과 생김새 설명만 쓴다.
- 스토리당 한 장이다. 카드를 고칠 때는 만들지 않고, 다시 플레이해도 만들지 않는다.
- 만든 표지는 공식 표지와 같은 저장 방식(표지 저장소, 내용 해시 이름, BlurHash)을
  쓴다.
...
- 상황 한 문장을 보내면 플린의 다음 말에 구체적인 사건이나 인물 제안이 있고, 질문은
  하나다. 화면에 선택지 버튼이 없다.
- 두세 번 오간 뒤 스토리 카드가 나온다. 카드에 제목, 한 줄 소개, 인물, 화별
  제목과 상황 설명, `대화 시작하기`가 있고 입력창은 열려 있다. 인물은 4명, 한 화의
  인물은 3명을 넘지 않는다.
- "2화는 빼 줘"를 보내면 새 카드의 화 수가 하나 줄고, 남은 화의 제목과 인물은
  이전 카드와 같다.
...
- `대화 시작하기`를 누르면 버튼이 진행 중을 보여 주고 두 번 눌리지 않으며, 끝나면 1화가
  ...
  목록에 방금 만든 스토리가 있다.
- 1화는 세 문장 이하의 한국어 장면 서술 뒤 영어 대사만으로 흐르고 지문이 없다.
  화자 이름은 카드의 인물 안에 있고 이름표 색은 카드의 인물 순서를 따른다. 결말은
  성공·타협·실패 중 하나로 난다.
- 1화에서 탐색으로 돌아오면 방금 만든 스토리에 표지가 보이고, 상세와 대화 기록에서도
  같은 표지가 보인다. 표지에 글자나 로고가 없고 인물 한 명의 상반신이다.
- 표지 생성만 실패시키면 스토리와 1화는 그대로 만들어지고 표지 자리는 빈 색 상자다.
...
- 칩은 HeroUI의 `TagGroup` 기본 패턴을 그대로 쓴다.
...
- **표지는 만들 때 같이, 가장 싼 모델로.** 같은 게이트웨이의 이미지 모델 세 개에
  공식 표지 스타일 문구와 베를린 스토리의 인물 설명을 주고 두 장씩 만들어 비교했다.
  비용은 게이트웨이가 응답에 실어 준 실제 청구액이다.
  ...
  사용자가 gpt-image-1-mini의 low 품질을 골랐다. 그림은 gpt-image-2가 가장 좋지만
  표지가 목록에서 작게 보여 low로 충분하고, 값이 10분의 1이다. 스토리 하나의 글
  생성이 약 $0.005이므로 표지를 더해도 스토리당 8원 안팎이다. 시점은 사용자가 "만들
  때 같이"로 정했다. 첫 메시지 시점으로 미루면 첫 장면만 보고 나간 스토리의 표지
  값을 아낄 수 있지만, 그만한 값이 아니고 만든 스토리를 화면에서 표지와 함께 보는
  쪽이 맞다고 판단했다.
...
- **구역 제목 대신 필터 칩.** 처음에는 `내가 만든 스토리`와 `플린이 만든 스토리`
  ...
  유튜브가 목록 필터에 쓰는 모양이고 값이 늘어도 이어 붙일 수 있다. 칩은
  HeroUI의 `TagGroup`을 기본 설정 그대로 쓴다. 지금 칩은 둘뿐이라 줄이 넘칠 일이
  없어 가로로 미는 장치를 따로 두지 않는다.
  칩 이름은 한국 앱의 `내 ○○` 관용(제타의 `내 캐릭터`, 밀리의서재의 `내 서재`)을
  따르고, `나만의 스토리`는 작은 화면에서 길어서 뺐다.
- **지문 없는 대화와 스토리 단위 인물을 그대로 따른다.** 이 스펙을 쓰는 사이에
  main에 [지문 없는 에피소드 대화](../episode-dialogue-only/spec.md)가 들어와
  [에피소드 콘텐츠 제작](../../decisions/episode-authoring.md)이 바뀌었다. 도입은 장면
  서술 한 덩어리, 그 뒤는 대사만, 인물은 스토리 단위이고 화당 1–3명, 스토리당
  4명이다. 만든 스토리가 같은 형식이어야 같은 플레이 규칙과 에피소드 화면을 쓸 수
  있으므로, 각본 규칙과 카드의 인물 수를 그에 맞췄다.
...
- 첫마디는 고정 문구다.
- 화 수 기본 3화, 상한 5화.
- 카드에 적힌 인물 순서가 스토리 안 순서다. 플린은 이야기의 중심 상대를 1번에 둔다.
- 표지 모델은 `openai/gpt-image-1-mini`, 품질 low, 1024×1024 한 장이다. 사용자가
  모델과 품질을 골랐고, 크기는 이 모델의 최소값이다. 결과물 형식과 저장 크기는
  구현이 정한다.
- 표지 스타일 문구는 공식 표지를 보고 적은 것으로 시작한다. 실제 결과를 보며 고친다.
...
- 표지 다시 만들기, 사용자가 표지를 고르거나 올리기. 첫 조각은 자동 생성 한 장으로
  끝난다.
...
- **플린이 카드를 내놓는 시점의 기준.** 몇 번 오간 뒤가 적당한지는 실제 대화를 보고
  ...
  삼는다.
- **공유와 공개.** 별도 제품 정의.
- **표지 다시 만들기.** 실패했거나 마음에 안 드는 표지를 다시 만드는 길은 dogfooding
  뒤에 정한다. 그때까지 실패한 표지는 빈 색 상자로 남는다.

## 남은 위험

- **각본 품질.** 측정에서 모델은 첫 장면은 seed와 비슷하게, 무대 지시는 seed의 3분의
  2 길이로 썼다. 사건이 약하거나 인물의 태도가 흐릿한 각본이 나올 수 있다. 공식
  seed를 형식 예시로 주는 것이 첫 대응이다. 측정은 지문이 섞인 옛 형식으로 했고,
  장면 서술과 대사만 쓰는 새 형식과 스토리 단위 인물 설명은 아직 재지 않았다.
  분량은 비슷해 시간은 크게 다르지 않겠지만, 인물이 사건과 결과를 말로만 전하는
  대사를 모델이 잘 쓰는지는 확인이 필요하다.
...
- **표지 품질.** low 품질은 명찰 글자가 깨지고 선이 조금 뭉개진다. 목록의 작은 표지
  에서는 눈에 띄지 않지만 상세처럼 크게 보이는 자리에서 거슬리면 gpt-image-2로 올린다.
  같은 문구로도 그림이 매번 달라 공식 표지와 결이 어긋날 수 있다.
- **표지 생성 실패.** 이미지 모델의 안전 필터나 일시 오류로 그림이 안 나올 수 있다.
  그때 스토리는 빈 상자로 남고 다시 만드는 길은 아직 없다.
...
- 2026-09-10 표지 생성 측정. 같은 게이트웨이로 gpt-image-1-mini(high, low),
  gpt-image-2, Nano Banana 2에 같은 문구를 주고 두 장씩 만들어 사용자가 직접 보고
  골랐다. 비용은 게이트웨이 응답의 청구액이고 표는 결정 이유에 있다. 프로토타입의
  만든 스토리 표지는 그때 mini low로 만든 그림을 줄인 것이다.
