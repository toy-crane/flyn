...
상태: 승인. 사용자가 2026-09-10에 인터뷰 방식, 스토리 카드, `플레이하기`에서
각본을 만드는 순서, 탐색 탭 진입을 채팅으로 확정했다. 비교 시안은 결정을 받은 뒤
버렸고, 확정한 흐름 전체(탐색의 + 버튼, 스토리 만들기 대화, 스토리 카드, 플레이하기,
...
- 탐색 목록은 두 구역이다. 위에 `내가 만든 스토리`, 아래에 `플린이 만든 스토리`를
  둔다. 만든 스토리가 없으면 위 구역을 두지 않는다. 옛 제목 `모든 스토리`는
  `플린이 만든 스토리`로 바꾼다.
...
- 플린이 충분히 알았다고 판단하면 정리 문장을 따로 쓰지 않고 스토리 카드를
  ...
  상황 설명 한 줄을 보여 주고, 아래에 Primary `플레이하기`를 둔다. 상황 설명은
...
- 만들기에 실패하면 [모바일 스토리 탐색](../../decisions/mobile-story-browsing.md)의
  ...
  시도는 `내가 만든 스토리`에도 회차에도 아무것도 남기지 않는다.
- `스토리 만들기` 화면을 나가면 대화와 카드는 사라진다. 다시 열면 첫마디부터
  시작한다.

### 만든 뒤

- 만든 스토리는 탐색의 `내가 만든 스토리`에 만든 순서의 역순으로 보인다. 표지
...
- **개요는 대화 중에, 각본은 `플레이하기`에서.** 사용자가 카드에서 판단하는 것은
  개요뿐이고 각본은 화면에 보이지 않는다. 각본을 확정 뒤 한 번만 만들면 카드를 몇
  번 고쳐도 비용이 들지 않고, 저장이 `플레이하기`로 미뤄져 버린 시도가 목록에 쌓이지
...
- `내가 만든 스토리`는 만든 순서의 역순이고, 없으면 구역을 숨긴다.
- 공식 목록의 새 제목은 `플린이 만든 스토리`다. `내가 만든 스토리`와 짝을 이룬다.
...
- **`플레이하기` 대기.** 3화 기준 13–18초다. 진행 표시만 있고 취소는 없다.
...
- 2026-09-10 비교 시안 세 번: 묻는 방식(전부 반려), 컴포넌트 범위(카드 하나),
  초안이 놓이는 자리(대화 안, 입력창 열림)와 여는 자리(탐색). 시안은 결정 뒤 버렸다.
