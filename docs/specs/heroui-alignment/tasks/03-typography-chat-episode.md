...
- [x] 표현 돌아보기의 `기억해 둘 표현` 제목이 접근성 트리에서 헤더 역할이다.
- [x] iOS와 Android 기기에서 결말 카드의 결과 문장, `기억해 둘 표현`, 배울 표현의 고친 영어 문장, `weight`가 붙은 강조 줄이 같은 화면의 본문보다 굵게 보인다.
- [x] 대화(종료 카드 포함), 표현 돌아보기, 에피소드를 열 수 없는 화면을 계약의 절차로 기본 크기와 최대 글자 크기, 밝은 화면과 어두운 화면으로 열어 잘림과 겹침이 없다.
...
- Verification: 코드 기준은 통과했다. 세 폴더 grep은 채팅 입력칸 `TextInput` 말고 0건이고, 남은 RN `Text`는 `shared/ui/marked-text.tsx`의 중첩 표시 `Text`(색·배경·밑줄 클래스)뿐이다. `bun run check`, `check-types`, 모바일 테스트가 통과했다. `mobile-ui-consistency-reviewer`(범위 `237ec54..ef5dc3b`)는 PASS_WITH_GAPS다. P3 두 건(회차 줄의 강조가 대응표 문장에 없음, 마지막 화 인사가 헤더로 읽힘)은 대응표 문장에 회차 줄과 마지막 화 인사를 더하고, 인사를 `Paragraph` `weight="semibold"`로 바꿔 닫았다. 2026-09-14 iOS 기본 크기·밝은 화면에서 대화(펼친 배울 표현 카드, 결말 카드)와 표현 돌아보기를 열어 잘림과 겹침이 없음을 봤다. 최대 크기, 어두운 화면, Android, 에피소드를 열 수 없는 화면은 아래 막힘으로 멈췄다.
  - 기기 확인(2026-09-14, HeroUI Native 1.0.9): iOS 기본 크기와 `accessibility-extra-extra-extra-large`, Android 기본 크기와 200%에서 각각 앱을 다시 시작한 뒤 밝은 화면과 어두운 화면으로 찍었다. 대화는 장면 서술(첫 줄 세미볼드), 인물 이름표, 사용자 말풍선, 펼친 배울 표현 카드(고친 영어 문장 h6 세미볼드, 바뀐 부분 강조색), 대사 뜻과 `AI에게 물어보기`, 표현 확인 실패 줄, 답변 오류 줄과 `다시 시도하기`, 결말 카드(`해냈어요!`와 결과 문장 h3)를 봤다. 표현 돌아보기는 이야기·회차 줄, `기억해 둘 표현`, 표현 카드, 다음 이야기 영역과 토스트를, 에피소드를 열 수 없는 화면은 API를 멈추고 대화를 열어 봤다. 잘림과 겹침은 없었다.
  - 최대 크기에서의 모습: iOS 대화는 상황 줄과 입력 영역이 화면 대부분을 차지해 목록이 짧은 창으로 보이고, 결말 카드는 계약대로 화면 높이 40% 안에서 글을 스크롤한다. 두 경우 모두 글은 스크롤로 닿고 서로 겹치지 않는다. 표현 돌아보기의 다음 이야기 영역도 높이 상한 안에서 스크롤된다. iOS 최대 크기의 대화 중 배울 표현 카드는 이 짧은 창으로 조각씩 확인했고 Android 200%에서는 카드 전체를 봤다.
  - 토스트: 최대 크기에서 문구 아래가 잘리던 것을 고쳤다(5418ecd). 인라인 줄 높이는 대응표 `body`의 28을 그대로 두고, React Native가 곱하는 확대 상한 배율에 한 번 더 곱하지 않는다. iOS 최대 크기와 Android 200%에서 잘리지 않음을 봤다.
  - 접근성 트리: Android 표현 돌아보기에서 `기억해 둘 표현`과 다음 화 제목 노드가 `android.view.View`(헤더 역할)이고 `우리 동네 카페`, 회차 줄, `다음 이야기`는 `android.widget.TextView`다. iOS는 `agent-device`가 헤더 특성을 내보내지 않아 jest `getByRole("header")`로만 확인했다.
- Blocker: 해소됨. 아래는 막혔던 당시의 기록이다. task 02의 Blocker와 같다. HeroUI Native 1.0.8 `Typography`의 굵기가 정의되지 않은 `--font-*` 변수에 기대어, 결말 카드 결과 문장(h3), `기억해 둘 표현`(h6), 고친 영어 문장(h6), 장면 첫 줄·인물 이름표·회차 줄(`weight`)이 모두 보통 굵기로 그려진다. 문구·색·배치가 그대로라는 이 task의 결과와 대응표의 굵기 위계가 기기에서 성립하지 않는다. iOS `agent-device`는 XCTest 노드에 헤더 특성을 내보내지 않아(`role="heading"` 선택자 실패, 노드 형식 `StaticText`) `기억해 둘 표현`의 헤더 역할은 Android 트리로 확인해야 한다.
- Revision: 대응표에 결말 카드 결과 문장(h3), 본문색 목록 제목(h6), 헤더가 아닌 줄의 `weight` 강조를 더했다. 대사 뜻 카드의 뜻은 헤더가 아니므로 `Heading h6`에서 `Paragraph` `weight="semibold"`로 바꿨다(ef5dc3b). `AI에게 물어보기` 링크는 `text-xs`를 떼어 `LinkButton` `sm` 라벨(14)이 크기를 정한다. [대사 뜻 계약](../../../decisions/mobile-utterance-translation.md)이 `sm` 크기를 정하므로 계약과 맞다. `shared/ui/marked-text.tsx`는 04의 폴더지만 대화·에피소드 호출부가 크기 클래스를 넘겨서 여기서 `type`과 `weight`를 받게 바꿨다. 04는 `shared/ui/expression-card.tsx`가 아직 넘기는 크기 클래스만 정리하면 된다. 2026-09-14 스펙 개정(2fb26ed)으로 1.0.9 올리기가 01에 들어가, 위 막힘은 01의 막힘으로 옮기고 굵기 기준을 더했다. 재개할 때 확인할 것: 토스트는 `lineHeight: 28 × min(fontScale, 1.6)`에 RN이 배율을 한 번 더 곱할 수 있어(c0a4bb1부터 있던 계산) 최대 크기에서 알약 높이를 본다.
