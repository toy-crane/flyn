...
- 원격 Supabase 프로젝트는 아직 없다. 앞으로 원격을 만들 때는 자동 노출을
  차단하는 새 기본값의 프로젝트로 만들고, 과거 기본값이 남은 프로젝트를
  재사용하지 않는다. security definer 함수는 소유자 권한으로 실행되어 RLS를
  지나치므로, 과거 기본값 원격에서는 REVOKE 없는 새 함수의 EXECUTE가 anon에게
  자동으로 열리기 때문이다. 원격을 만들 때 이 차단 상태를 한 번 확인한다.
