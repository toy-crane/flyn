...
- 콘텐츠는 **가운데 정렬**. Google 스펙의 "왼쪽 16pt"는 좌측 정렬 변형의
  값이라 여기서는 적용하지 않는다.
- **로고 → 라벨 간격도 Apple 실측 16%로 통일한다.** Google 스펙은 12pt를
  명시하지만, "Apple에 맞춰 Google도 맞춘다"가 이 절의 목적이고 눈으로 보면
  간격 차이가 가장 먼저 드러난다. 이미 Google Sans를 못 써서 한 번 벗어난
  터라, 같은 성격의 미세 이탈을 하나 더 받아들인다.
- **주의 — 컨테이너 `gap`과 로고 마진이 겹쳐 더해지기 쉽다.** 프로토타입에서
  실제로 이 버그가 났다(Apple 12px, Google 20px). 소셜 버튼은 컨테이너
  `gap`을 0으로 두고 로고 마진 하나로만 간격을 만든다.
...
- **에러는 화면에 따라 다른 곳에 뜬다.** 하나로 통일하려던 앞선 안을 뒤집는다.
  - **`sign-in`(소셜 실패) → `Alert`.** Apple·Google 실패는 폼 검증이 아니라
    **OS 시트가 닫히면서 돌아오는 모달 흐름의 결과**다. 시트가 사라진 자리에서
    버튼 아래 작은 빨간 줄은 놓치기 쉽고, 이럴 때 iOS 관용은 얼럿이다.
    제목 "로그인하지 못했습니다" + 한 문장 + `확인` 하나.
  - **`email`·`code` → 인라인 각주.** 이쪽은 입력에 붙은 검증 결과이고
    사용자의 시선이 이미 필드에 있다. `systemRed` 각주를 필드·버튼 옆에 둔다.
- **원문 에러 메시지를 그대로 노출하지 않는다.** 현재의
  `로그인 실패: ${error.message}`는 "Error:" 접두사에 벤더의 영문·기술 문구를
  얹어 내보낸다. 알려진 경우(네트워크, 레이트리밋, 코드 불일치, 코드 만료)를
  한국어 문장으로 매핑하고 나머지는 일반 문구로 떨어뜨린다. **원문은 UI가
  아니라 콘솔·PostHog로 보낸다** — 진단 가치는 거기서 챙긴다.
- `ERR_REQUEST_CANCELED`는 지금처럼 조용히 무시한다. 얼럿도 띄우지 않는다.
...
### 7. 코드 입력 — 6칸으로 보이되 필드는 하나 (RN으로 만든다)
