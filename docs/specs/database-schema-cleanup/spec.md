...
- 최초 검증에서 iOS는 다른 `agent-device` 세션의 사용권과 충돌해 조작하지 못했다. [개발 기기 사용권 충돌](../../follow-ups/dev-session-assigns-agent-device-owned-simulator.md)에 원인과 복구 과정을 기록했다. 아래 추가 검증으로 iOS 화면 확인도 마쳤다.
...
### iOS 추가 검증 (2026-09-14)

- 사용자가 동시 작업 세션이 없음을 확인한 뒤 오래된 도구 daemon을 종료했다. `flyn-db-cleanup-ios`가 이 worktree에 배정된 `flyn-slot-1`의 사용권을 얻었다. 앱·기기를 초기화하지 않았으며 새 로컬 이메일 테스트 계정으로 로그인했다. Metro `8092`의 실제 연결 목록에서 `com.odd.flyn (flyn-slot-1)`을 확인했다. 검증 대상 제품 코드는 `01586ea`와 같다.
- `우리 동네 카페` 1화에서 영어 입력의 교정과 저장·취소·재저장, 대사 저장 후 번역 열기, 한국어 입력의 영어 안내 저장, 자연스러운 표현 판정, 대화 종료, 표현 돌아보기와 2화 진입을 확인했다. 실제 모델과 로컬 API·Supabase를 사용했다. 입력 후 입력창이 비워지고, 취소 후에도 교정 카드가 유지됐다.
- 2화의 Mia·Owen 대사를 각각 번역하고 Mia만 저장했다. 앱 프로세스를 재시작한 뒤 대화 기록에서 다시 열어 같은 번역과 저장 표시를 확인했다. 실제 DB에도 메시지 한 행과 위치 `{0,1}`의 표현 두 행, 저장 여부 `{true,false}`가 있었다. 끝난 1화를 다시 열었을 때 자연스러운 표현 판정도 유지됐다.
- 노트에서 대화·돌아보기와 같은 내용 및 최근 저장 순서를 확인했다. 노트의 Mia 항목을 삭제하고 2화를 다시 열자 Mia의 저장 표시만 해제되고 두 번역은 그대로 남았다. API·Metro 로그에 관련 실행 오류가 없었고 검증 중 앱 오류 화면이나 비정상 종료도 없었다.
- 화면 증거는 `/tmp/flyn-db-ios-saved-translation.png`, `/tmp/flyn-db-ios-review.png`, `/tmp/flyn-db-ios-reopened.png`, `/tmp/flyn-db-ios-note.png`, `/tmp/flyn-db-ios-note-deleted-translation-kept.png`에 남겼다. 이번 추가 검증은 위 수락 기준 표의 Android 화면 근거에 대응하는 iOS 근거다. 동시에 두 기기를 조작하는 검사는 추가하지 않았다.
- 설정에서 로그아웃한 뒤 `이메일로 계속하기`가 다시 보이는 것을 확인하고 같은 검증 세션을 닫았다. API와 Metro는 실행해 두었다. 제품 코드 변경 없이 검증 기록만 추가했다.
