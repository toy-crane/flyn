...
2026-09-14 사용자가 공통 시각 열, 정리 후보와 표현 저장 통합을 승인했다. AI 응답 묶음과 그 안의 대사 위치는 유지하기로 했다. 이 명세는 구조 변경의 계약이며 구현 완료 기록이 아니다.
...
- [검증과 내부 테스트 배포](../../decisions/continuous-delivery.md): 배포 순서와 기존 클라이언트 호환.
...
| `kind` | 인물 대사, 영어 교정, 한국어 입력의 영어 안내 |
| `text`, `meaning` | 표시할 문장과 뜻 |
| `episode_id`, `message_id` | 에피소드와 출처 메시지. 메시지는 삭제 뒤 비어 있을 수 있다 |
| `utterance_index` | 인물 말풍선의 0부터 시작하는 위치. 사용자 메시지의 배울 표현은 `NULL` |
...
검사 완료 상태는 표현 존재 여부로 추측하지 않는다. `natural`, `unclear`는 다시 열어도 완료 상태를 유지하며 카드와 노트 항목을 만들지 않는다. `corrected`는 필요한 표현 내용과 함께 확정된다. 서로 다른 계정의 메시지나 에피소드를 출처로 연결할 수 없다.
...
| `utterance_at` | `utterance_index`로 통일. 기존 0부터 시작하는 대사 위치를 보존한다 |
...
- 기존 검사 결과의 판정과 카드 내용, 예문·예문 뜻을 보존한다. 이행 중 모델을 다시 호출해 내용을 바꾸지 않는다.
...
- DB·API·설치된 앱의 버전 차이 때문에 번역, 담기와 대화 조회가 실패하지 않아야 한다. 배포 계약에 따른 호환 기간은 허용하지만 중복 데이터의 영구 유지는 최종 상태로 인정하지 않는다.
...
7. 교정·안내는 대화, 돌아보기, 노트가 같은 내용을 읽는다. 문제없음·뜻을 알 수 없음·실패·처리 중은 구별되며 카드의 부재만으로 재생성 여부를 판단하지 않는다.
...
11. 최종 구조와 소비자에서 제거한 중복 데이터와 옛 이름이 사라진다. 스키마·보존 검사와 실제 번역·교정·담기·취소·다시 열기 검증이 통과한다. 검사 방식은 필수 결정 계약을 따른다.

## 가정과 남은 위험

- 실행자가 정한 기본값: 기존 `episode_expression_results`는 검사 완료 판정만 보관하는 용도로 유지하고 카드 내용은 `expressions`가 소유한다. 카드 없는 완료 결과를 가짜 표현으로 만들지 않는 가장 작은 변경이다. 두 내용의 확정은 여전히 원자적이어야 한다. 이 기본값은 재사용 결정 계약이 아니며 구현에서 더 단순한 방법이 확인되면 같은 관찰 가능한 조건 안에서 바꿀 수 있다.
