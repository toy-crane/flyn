...
- Verification: `bun run check-types` 5/5, `apps/api` `bun test` 52/52,
  ...
  코드 리뷰에서 네 가지를 찾아 고쳤다. 모델이 쓴 빈 기록 줄이나 300자를 넘는
  줄이 결말 기록 전체를 무너뜨려 사건이 끝나도 화면이 닫히지 않던 것, 헤더
  뒤로 가기로 나갈 때 홈이 지난 상태로 남아 다음 화가 409로 막히던 것, 홈이
  읽는 중에도 실패를 알리던 것, 언어 수준 덮어쓰기 경로에 검사가 없던 것이다.
  두 번째는 기기에서 다시 확인했다. 에피소드 화면에 있는 동안 진행을 바꾸고
  헤더 뒤로 가기로 나가니 홈이 곧바로 다음 화로 갱신됐다.
