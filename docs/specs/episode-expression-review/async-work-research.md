...
마지막 캐릭터 대사는 바로 보여 주고, 해당 대화의 모든 표현 확인이 끝나면 축하 연출과 종료 카드, `표현 돌아보기` 버튼을 표시한다. 표현 확인은 영어 교정과 한국어 입력의 영어 안내를 만드는 작업이다. 사용자가 카드를 읽는 행동을 기다리는 것이 아니다.

아래의 백그라운드 실행, 재시도와 UI 적용안은 조사에 따른 제안이다. 아직 승인된 요구사항으로 취급하지 않는다.
...
핵심 후보는 **화면을 떠나도 서버가 이미 접수한 표현 확인을 계속하고, 돌아오면 같은 대화에서 최신 상태를 보여 주는 방식**이다.
...
다음에 정할 것은 화면을 나가도 작업을 계속하는 방향을 채택할지다. 그 뒤 실패 시 종료를 얼마나 보류할지와 재시도 후의 경험을 결정하고 대기 UI를 프로토타입에서 확인한다.
...
추천을 조정한다. 이번 범위에서는 화면 밖의 지속 실행을 보장하는 기능을 넣지 않고, 돌아왔을 때 완료되지 않은 메시지만 자동으로 다시 확인하는 방식을 먼저 검토한다. `자연스러운 표현`도 완료로 남겨 반복 검사하지 않아야 한다. 마지막 대사를 다시 생성하거나 사용자의 메시지를 다시 보내는 동작은 필요하지 않다. 이 대안은 아직 사용자가 선택하지 않았다.
