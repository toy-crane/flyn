...
상태: 제품 동작 정의 완료 · 구현 전. 2026-09-10까지 확인한 결정과 시안의 기본값을 기록한다.
...
2026-09-10 필터 변경 후 320·390·768px에서 기본·일부 실패·전체 실패 화면을 확인했다. 일부 실패에서는 준비된 카드만 남고, 전체 실패에서는 제목·개별 재시도 없이 빈 상태가 표시되며 실제 실패 판정은 유지된다. 독립 검토에서 기본 카드의 글꼴·간격·줄바꿈이 참조 `0a11176`과 육안상 같음을 확인했다. 목록 조회 오류의 재시도와 대화 화면의 개별 재시도도 유지된다. 실제 OS의 글자 크기 설정과 API·저장 검증은 구현 단계에 남는다.
