...
- 먼저 준비된 표현은 바로 보여 준다. 아직 확인 중인 표현은 준비되는 대로 같은 목록에 추가한다. 모든 표현이 준비될 때까지 대화 종료나 표현 화면 진입을 미루지 않는다.
...
1. **표현의 확인이 끝나지 않거나 실패한 경우.** 준비된 카드부터 보여 주고 늦은 결과를 추가하는 방향은 확인했다. 일부 확인 실패와 다시 확인, 확인 중 화면을 나갔다 돌아온 경우의 처리는 아직 정하지 않았다. 현재 시안의 `마지막 메시지의 표현을 확인하고 있어요.`와 빈 결과를 함께 표시하는 모양은 변경해야 한다. 시안에서 실제 완료 전환을 확인한 상태는 아니다.
2. **기존 기록의 추가 내용.** 이미 저장한 교정에는 새로 정한 상황, 전체 문장의 한국어 뜻과 다른 예문이 없다. 이전 기록을 열 때 이 내용을 보충할지, 기존 정보까지 보여 줄지는 아직 정하지 않았다. 새로운 카드 내용을 저장하는 결정만으로 이전 기록도 완성되었다고 보지 않는다.
...
- 표현 확인이 일부만 끝났어도 준비된 카드를 볼 수 있다. 나머지 결과가 도착하면 같은 목록에 추가하며 대기 중인 항목을 확정된 빈 결과로 표시하지 않는다.
...
- 일부 실패, 화면 이동 뒤의 확인과 기존 기록의 추가 내용에 대한 완료 기준은 위 미정 사항을 정한 뒤 추가한다.
