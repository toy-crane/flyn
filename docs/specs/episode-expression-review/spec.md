...
종료 표시 순서는 마지막 대사, 모든 표현 확인 완료, 축하와 종료 카드 순으로 확인했다. 남은 주제는 확인이 실패하거나 사용자가 화면을 나간 경우의 처리다.

- 사용자는 화면 밖에서도 표현 확인을 계속하는 기능은 쉽게 구현할 수 있을 때만 원한다. 구현 단순성을 우선하며, 이 기능을 필수 범위로 보지 않는다. 지속 실행 대신 재진입 때 미완료 메시지를 다시 확인하는 대안은 아직 제안이다.
- 일부 표현 확인 실패와 재시도, 판정할 수 없는 입력, 확인 중 화면을 나갔다 돌아왔을 때의 처리를 정한다. 실패를 확인 완료로 취급하거나 무한히 기다리게 하는 방식을 확정하지 않았다.
- 현재 확정한 것은 사용자에게 종료를 보여 주는 순서다. 서버가 결말을 기록하는 시점, 화면 밖에서 작업을 이어가는 방식과 재시도 정책까지 합의한 것은 아니다.
- 현재 시안의 `마지막 메시지의 표현을 확인하고 있어요.`와 빈 결과를 함께 표시하는 모양은 확정안이 아니다. 표현 확인 완료와 종료 사이의 전환도 아직 시안에서 검증하지 않았다.

[비동기 처리와 UI 조사](async-work-research.md)에 공식 문서, 수집 화면과 적용 후보를 기록했다. 조사 문서의 제안은 승인된 요구사항이 아니다.
...
- 표현 확인의 일부 실패와 화면 이동 뒤의 처리 기준은 위 미정 사항을 정한 뒤 추가한다. 합의한 종료 표시 순서도 프로토타입에서 전환을 확인해야 한다.
