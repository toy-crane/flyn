...
- 개별 메시지의 표현 확인 실패는 목록 조회 실패와 구분한다. 준비된 표현 카드와 함께 실패한 원문, 기존 실패 안내와 재시도를 보여 준다. 모든 메시지의 확인이 실패했어도 원문과 재시도를 유지하며, `안내한 표현이 없어요`로 표시하지 않는다.
...
- 재시도 중에는 해당 메시지에 진행 표시를 보여 준다. 성공하면 저장한 결과로 바꾸고, 다시 실패하면 실패 안내와 재시도로 돌아온다. 요청 제한 시간에 도달한 경우에도 진행 표시를 계속 돌리지 않고 기존 오류 처리로 끝낸다.
...
| 표현 확인 실패 | 실패한 원문과 재시도를 유지하면서 종료 카드와 표현 돌아보기를 사용할 수 있다. 재시도하면 준비된 표현과 함께 결과가 갱신된다. |
| 모든 표현 확인 실패 | 표현 없는 정상 결과 대신 실패한 원문과 개별 재시도를 보여 준다. |
| 재시도도 실패 | 재시도 뒤 같은 오류 안내로 돌아오고 자동으로 요청을 반복하지 않는다. |

시안의 일반 확인은 약 1.4초, 지연 미리보기는 약 12초의 가상 타이머를 사용한다. 실제 요청의 성능 목표나 제한 시간을 정한 값이 아니다.
