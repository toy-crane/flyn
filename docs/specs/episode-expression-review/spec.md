...
- 대화 중 표현 안내는 준비되는 대로 보여 준다. 종료 직후의 목록에 늦은 표현을 추가하는 방식은 아래의 종료 시점 논의로 재검토한다. 표현 확인보다 먼저 종료해도 된다는 이전 가정을 구현 기준으로 삼지 않는다.
...
남은 주제는 표현 확인과 에피소드 종료의 시점이다. 사용자는 표현 확인이 끝나기 전에 대화를 종료하는 방식에 이의를 제기하고 대안을 비교하기로 했다.

- 마지막 대사는 먼저 보여 주고 종료 카드를 기다리게 할지, 종료 카드는 보여 주되 표현 돌아보기 진입을 기다리게 할지 등 대기 위치를 논의한다. 어느 안도 아직 선택하지 않았다.
- 일부 표현 확인 실패와 다시 확인, 확인 중 화면을 나갔다 돌아왔을 때의 처리도 이 주제에서 정한다.
...
- 대화 중에는 먼저 준비된 표현 안내를 볼 수 있다. 확인 중인 항목을 확정된 빈 결과로 표시하지 않는다. 종료 카드와 표현 돌아보기의 표시 시점은 남은 결정을 반영한 뒤 검증한다.
...
- 표현 확인의 대기 위치, 일부 실패와 화면 이동 뒤의 처리 기준은 위 미정 사항을 정한 뒤 추가한다.
