...
- 화면 제목은 `표현 돌아보기`, 준비된 표현 카드의 목록 제목은 `기억해 둘 표현`이다. 이 제목은 카드가 있을 때만 표시한다. 제목 옆의 개수는 [표현 화면 다듬기](../expression-ui-refresh/spec.md)의 `기억해 둘 표현`이 없앴다(2026-09-10). 카드가 몇 장인지는 목록이 그대로 보여 준다.
- 영어 교정과 한국어 입력에 대한 영어 안내를 한 목록에서 보여 준다. `안 본 표현` 표시나 읽었는지에 따른 구분을 두지 않는다.
- 카드에는 쓰는 상황, 영어 표현, 한국어 뜻을 보여 준다. 펼쳤을 때 보이는 것은 [표현 화면 다듬기](../expression-ui-refresh/spec.md)가 대신한다(2026-09-10). 표현 노트와 같은 카드를 쓰고, 펼치면 `내가 쓴 문장`과 `이렇게 쓰는 이유` 두 섹션만 보인다. 다른 예문과 그 뜻은 화면에서 빠진다. 카드 아래 오른쪽에는 복사와 책갈피의 아이콘 줄이 선다.
...
- 축하 연출은 [표현 화면 다듬기](../expression-ui-refresh/spec.md)의 `결말 축하`가 대신한다(2026-09-10). 마크와 조각은 프로젝트가 소유하는 Lottie 파일이고, 카드, 마크, 체크, 고리, 조각, `해냈어요!`, 결과 문장이 순서대로 나타난다. 목표를 이루지 못한 결말은 `해냈어요!`와 고리 없이 조각이 절반이다. 반짝임 아이콘과 이모지는 축하에 쓰지 않는다. 시각 값은 실제 기기에서 조정할 수 있다.
...
- 준비된 카드가 있을 때만 `기억해 둘 표현`을 표시한다. 카드가 없으면 빈 상태 안내를 보여 주고 제목은 표시하지 않는다. 일부 실패 시 준비된 카드만 남으며, 전체 실패 시에도 실패 안내나 개별 재시도가 새로 나타나지 않는다.
...
| 모든 표현 확인 실패 | 표현 돌아보기에는 기존 빈 상태 안내만 보여 주고 카드 제목과 개별 재시도는 표시하지 않는다. |
| 표현 없음 | 카드 제목 없이 기존 빈 상태 안내만 보여 준다. |
| 재시도도 실패 | 대화 화면의 재시도 뒤 같은 오류 안내로 돌아오고 자동으로 요청을 반복하지 않는다. 표현 돌아보기에서는 계속 해당 문장을 제외한다. |
| 표현을 불러오는 중 | 저장된 목록을 읽는 동안 본문에 진행 표시가 나타난다. 카드 제목과 빈 상태 안내는 숨기며 이동은 유지한다. |
