...
상태: 제품 동작 정의 완료 · 구현 전. 2026-09-09까지 확인한 결정과 시안의 기본값을 기록한다.
...
- 요청이 실패하면 진행 표시를 끝내고 그 메시지에 실패 안내와 재시도를 남긴다. 다른 요청이 모두 끝났다면 종료 카드를 표시한다. 실패한 메시지를 확인 완료나 자연스러운 표현으로 처리한다는 뜻은 아니다.
...
- 개별 메시지의 표현 확인 실패는 목록 조회 실패와 구분한다. 준비된 표현 카드를 먼저 보여 주고, 실패한 원문과 기존 실패 안내·재시도는 그 아래 별도 영역에 둔다. 영어 교정과 한국어 입력의 영어 안내는 계속 같은 카드 목록에 모은다.
- 모든 메시지의 확인이 실패했으면 표현 카드의 목록 제목과 개수를 숨기고, 실패한 원문과 재시도를 보여 준다. `안내한 표현이 없어요`로 표시하지 않는다.
- 정상적으로 안내할 표현이 없으면 기존 빈 상태 안내만 보여 준다. 표현 카드의 목록 제목과 `0개`는 표시하지 않는다.
- 조회나 개별 재시도 중에도 뒤로 가기, 홈과 하단 이동 버튼을 사용할 수 있다. 재조회 결과가 도착해도 사용자가 이동한 다른 화면이나 화의 내용이 바뀌지 않는다.
...
- 재진입 뒤 다시 확인한 요청도 실패할 수 있다. 그때는 실패 안내와 재시도를 남기고 종료를 허용한다. 같은 화면에서 실패한 요청을 무한히 자동 재시도하지 않는다.

### 표현 확인 실패와 재시도

- 실패한 메시지 곁에 기존의 `표현을 확인하지 못했어요.`와 재시도 아이콘을 표시한다. 아이콘의 접근성 이름은 `표현 다시 확인`이다. 대화와 표현 돌아보기에서 같은 메시지의 상태를 사용한다.
- 확인 중인 요청이 남아 있으면 기다리고, 요청이 성공 또는 실패로 모두 끝나면 종료 카드와 `표현 돌아보기`를 표시한다. 일부 또는 모든 표현 확인이 실패해도 대화의 결말을 유지하며 다음 화와 홈으로 이동할 수 있다.
- 사용자가 재시도를 누르면 그 메시지의 표현 확인만 다시 수행한다. 기존 대화, 결말과 준비된 표현은 유지하며 사용자 메시지와 캐릭터 답변을 다시 만들지 않는다.
- 재시도 중에는 해당 메시지에 진행 표시를 보여 준다. 성공하면 저장한 결과로 바꾸고, 다시 실패하면 실패 안내와 재시도로 돌아온다. 돌아보기에서 교정 결과가 준비되면 해당 문장은 위의 표현 카드 목록에 반영한다. 자연스럽다는 결과라면 실패 영역에서 제거하고 표현 카드를 만들지 않는다. 요청 제한 시간에 도달한 경우에도 진행 표시를 계속 돌리지 않고 기존 오류 처리로 끝낸다.
...
종료 표시 순서, 재진입 시 미완료 메시지 재확인, 실패 시 종료 허용과 개별 재시도를 확인했다. 이번 범위에서 추가로 선택해야 할 제품 동작은 남아 있지 않다. 아래의 시안 기본값은 구현 과정에서 조정할 수 있다.
...
- 준비된 카드 아래에 실패한 문장이 있으면 보조 제목 `다시 확인할 문장`으로 구분하는 것을 시안의 기본값으로 삼는다. 카드가 하나도 없으면 이 보조 제목도 생략하고 원문과 각 상태를 바로 보여 준다. 실패한 문장끼리는 대화 순서를 유지한다.
...
- 표현 돌아보기에서 재시도하는 동안 기존 카드와 화면 이동을 유지한다. 에피소드 화면에서 재시도하면 확인이 진행되는 동안 하단에 진행 표시를 보여 주고, 요청이 끝나면 정지된 종료 카드로 돌아온다.
...
- 일부 또는 모든 표현 확인이 실패해도 종료 카드와 표현 돌아보기로 이동할 수 있다. 돌아보기에는 실패한 원문과 재시도가 남고, 준비된 표현 카드는 유지된다. 전체 실패를 표현이 없는 정상 결과로 표시하지 않는다.
- 준비된 카드가 있을 때만 `기억해 둘 표현`과 카드 개수를 표시한다. 실패한 원문은 카드 개수에 포함하지 않으며 카드 아래에 구분해서 보여 준다. 전체 실패와 정상 빈 상태에서는 해당 제목과 개수를 표시하지 않는다.
- 목록을 읽는 동안에는 본문 진행 표시를 보여 주고 빈 결과로 단정하지 않는다. 조회 실패 후 다시 시도하면 오류 안내와 이동 버튼은 유지하고 재조회 버튼만 진행 중으로 바뀐다. 중복 누름이 새 요청을 만들지 않는다.
- 목록 재조회는 기존 메시지의 확인 결과를 바꾸지 않는다. 다시 읽어도 완료된 카드, 자연스러운 판정과 실패한 원문은 각 상태를 유지하며, 중단된 조회 결과가 다른 화에 나타나지 않는다.
- 재시도 시 해당 항목만 진행 표시로 바뀐다. 성공한 결과는 대화와 표현 화면에 함께 반영하고, 다시 실패하면 같은 자리에서 재시도할 수 있다. 실패가 이어져도 요청을 무한히 반복하지 않는다.
...
| 표현 확인 실패 | 준비된 카드와 개수 아래에 실패한 원문과 재시도를 구분한다. 재시도하면 해당 결과만 갱신된다. |
| 모든 표현 확인 실패 | 카드 제목과 개수 없이 실패한 원문과 개별 재시도를 보여 준다. |
| 표현 없음 | 카드 제목과 `0개` 없이 기존 빈 상태 안내만 보여 준다. |
| 재시도도 실패 | 재시도 뒤 같은 오류 안내로 돌아오고 자동으로 요청을 반복하지 않는다. |
...
2026-09-09 브라우저 검증에서는 전송→대기→종료, 일부·전체 실패, 개별 재시도의 성공·실패, 기록 재진입과 완료 결과 유지, 중복 카드·축하 반복 방지를 확인했다. 독립 검토에서도 기존 `879dcfc` 시안의 정상 완료·다른 결말·돌아보기·마지막 화 배치를 비교해 주요 차이나 동작 결함을 발견하지 않았다. 320·390·768px, 브라우저에서 글자 150% 확대, 어두운 화면과 동작 줄이기를 확인했다. 실제 OS의 글자 크기 설정과 API·저장 검증은 구현 단계에 남는다.

상태별 구성 보완 후에는 준비된 카드와 실패 영역의 구분, 빈 상태·전체 실패의 제목 숨김, 목록 조회·재시도 진행 표시, 조회 중 이동과 복귀를 추가 확인했다. 변경한 화면은 320·390·768px에서 확인했고, 320px의 일부 실패 화면은 글자 150% 확대 후에도 재시도에 접근할 수 있었다. 독립 검토에서는 참조 `3233bd0`의 기존 화면 12개 좌표 중 11개 이미지가 일치했다. 390px 정상 돌아보기의 이미지 비교는 캡처가 불완전해 제외했으며, 모든 이동 조합을 반복 검증한 것은 아니다.
