...
추가 요청은 가장 최근 카드의 episodes에 있는 현재 화 수를 기준으로 판단한다. 이미 5화라면 새 장면을 제안하기 전에 최대 5화라 추가할 수 없음을 알린다. 사용자가 삭제나 교체를 요청하지 않았다면 기존 화를 그대로 둔다. 대화로 장면을 제안하는 것도 추가이므로, 도구 호출만 막는 것으로 이 상한을 지켰다고 보지 않는다.
