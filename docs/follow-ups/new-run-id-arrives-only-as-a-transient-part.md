...
스트림의 `data-run-started` 일시 part다. 그 part를 놓치면 두 가지가 일어난다.
...
- `apps/api/src/features/episode/route.ts`의 `POST /`가 회차를 만든 뒤
  `data-run-started`를 `transient: true`로 쓴다. 일시 part는 메시지에 남지 않아
...
창은 좁다. `data-run-started`는 모델 글자보다 먼저 나가므로, 응답이 시작한 지
...
`story_runs`의 insert grant에 `id`를 더하고, 서버는 앱이 준 id로 회차를 만든다.
다시 보낸 요청이 같은 id를 들고 오면 이미 있는 회차를 그대로 쓴다.

그전까지 값싼 보완은 `runId`가 비었을 때 다음 화로 넘어가지 않고 시작 실패
