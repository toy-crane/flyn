...
**Proposed next step**: 실제 VoiceOver를 켜고 이 칩을 훑어 선택 상태와 누를 수 있다는 것이 전해지는지 확인한다. 전해지지 않으면 HeroUI Native에 `TagGroup.Item`이 선택 가능한 항목의 역할을 갖게 해 달라고 요청하거나, 앱이 역할을 덮어쓸 수 있는 통로를 요청한다. 같은 화면의 큰 글자 크기 확인은 [모바일 타이포그래피](../decisions/mobile-typography.md)의 절차로 함께 본다.
