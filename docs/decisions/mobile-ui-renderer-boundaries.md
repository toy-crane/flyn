...
- 에피소드는 사용자 검토로 정한 예외다. 불투명한 네이티브 헤더 아래에 파란 상황 줄을
  고정한다. 헤더에 화면 배경색을 지정하고 상단 스크롤 경계 효과를 끈다.
  스토리 상세와 다른 화면의 투명 헤더는 유지한다.
...
- iOS 채팅 입력창 뒤에서 대화가 점차 흐려지는 하단 배경은 떠 있는 기능 영역의
  표현으로 허용한다. 범위는 [모바일 AI 채팅 표현](mobile-ai-chat-rendering.md)을
  따르며 카드나 메시지 자체에 흐림 효과를 적용하는 근거로 쓰지 않는다.
- 입력이 끝난 대화에서 하단에 고정한 완료 안내도 뒤의 대화를 가리는 배경을 둔다.
  안내 문구와 최신 메시지 버튼은 배경 효과보다 위에 선명하게 표시한다.
