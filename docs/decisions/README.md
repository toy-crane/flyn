...
- [모바일 타이포그래피](mobile-typography.md) — 모바일 텍스트 역할, 시스템 폰트, monospace 사용, Dynamic Type, 시스템 글자 크기의 검증 절차와 지원 범위 또는 렌더러별 타이포그래피 매핑을 변경할 때 읽는다.
...
- [스토리 만들기](story-creation.md) — 사용자가 플린과 대화해 스토리를 만드는 흐름, 스토리 카드, `대화 시작하기`의 각본·표지 생성 또는 만든 스토리의 수명을 바꿀 때 읽는다.
...
- [모바일 에피소드 장면 표시](mobile-episode-scene-display.md) — 에피소드의 장면 서술 모양, 인물 이름표, 인물 구분 방식 또는 대화 화면의 색 규칙을 변경할 때 읽는다.
- [에피소드 종료와 표현 돌아보기](episode-ending-and-review.md) — 에피소드의 종료 순서, 결말 축하, 표현 돌아보기 화면, 카드 내용의 생성과 저장, 재진입이나 기록에서 다시 보기를 바꿀 때 읽는다.
- [표현 노트](expression-note.md) — 인물 대사나 배울 표현을 담는 동작, 저장 항목 또는 `노트` 탭의 목록과 카드를 바꿀 때 읽는다.
