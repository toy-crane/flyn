...
- 앱과 서버를 따로 배포해도 이전 앱의 요청 계약은 보존한다. 배포 순서와 호환성은 [검증과 내부 테스트 배포](continuous-delivery.md), 최소 지원 버전의 화면 차단은 [모바일 업데이트 경험](mobile-update-experience.md)을 따른다. 앱 화면 차단은 서버의 권한 검사나 구버전 요청 거절을 대신하지 않는다.
...
- 이전 앱의 요청 형식을 더 이상 유지할 수 없을 때. 서버의 구버전 요청 거절이나 API 버전 구분은 배포 계약과 함께 정한다.
