...
- 배포 주기를 나눈다는 것은 요청 계약까지 나눈다는 뜻이 아니다. 경로에 버전이 없고 최소 버전이나 강제 업데이트 장치도 없으므로, 요청이나 응답의 모양을 바꾸는 변경은 앱과 서버를 함께 내보내야 한다. 아직 배포한 빌드가 없어서 지금은 이 순서만 지키면 된다. 사용자 손에 빌드가 들어가는 날에는 이 방식으로는 부족하다.
...
- 배포한 빌드가 사용자 손에 남기 시작할 때. 그때부터는 앱과 서버를 함께 내보내는 것으로 요청 계약을 지킬 수 없다. 경로에 버전을 붙일지, 최소 버전을 요구할지, 옛 모양을 한동안 함께 받을지를 그 시점에 정한다. [모바일 개발 런타임](mobile-development-runtime.md)이 TestFlight와 스토어 배포를 따로 정하기로 미뤄 두었으므로 그 결정과 같이 다룬다.
