...
- 기본 실행과 에이전트의 자동 검증에서 가상 기기를 시작하거나 다시 연결할 때 현재 사용 중인 앱의 포커스를 빼앗지 않는다. iOS Simulator는 백그라운드로 열고 Android Emulator는 창을 숨겨 시작한다. 화면 조작과 캡처는 기기를 지정해 계속 사용할 수 있다.
- 사용자가 직접 검증하거나 화면 표시를 요청할 때만 `bun run dev <ios|android> --foreground`로 요청한 플랫폼의 기기 창을 앞으로 가져온다. 옵션은 이번 명령에만 적용하며 상태 파일에 저장하지 않는다. 실기기에는 사용하지 않는다. iOS는 해당 UDID의 별도 Simulator 창을 재사용하거나 연다. Android는 이미 보이는 프로세스를 활성화하고, 숨겨 시작한 프로세스는 해당 AVD만 데이터 삭제 없이 창 모드로 재시작한다.
...
- 기기는 플랫폼별 최대 5개까지 필요할 때 만든다. 기존 배정을 우선하고, 없으면 풀의 빈 기기를 배정한다. 모두 배정 중이면 새 기기를 만들지 않고 `dev:status`와 `dev:remove`를 안내한다. 기기 수와 worktree slot 번호는 별개다. 기존 기기를 교체하거나 미리 5개를 만들지 않는다.
...
- 플린 앱 데이터와 로그인 상태는 공용 빌드에 포함하지 않는다. 각 worktree에 배정한 Simulator 또는 AVD가 독립적으로 소유한다.
...
- `apps/mobile`과 `apps/api`의 개별 `dev`, `ios`, `android`, `start` 명령은 사람이 하는 수동 진단과 실기기에 Development Build를 처음 설치하는 사전 준비에만 쓴다. 이 명령은 worktree 간 포트와 기기 격리를 보장하지 않으므로 에이전트가 화면을 검증할 때는 쓰지 않는다.
...
- `bun run dev:remove`는 현재 worktree에 배정된 두 플랫폼의 기기에서 플린 앱만 삭제하고 종료한 뒤 풀로 돌려놓고 slot을 반납한다. 기기의 Apple·Google 계정과 설정은 보존한다. 이미 플린 앱이 없으면 반납을 계속한다. 앱 삭제나 기기 종료가 실패하면 해당 기기의 반납과 재배정을 중단한다. 먼저 반납을 마친 기기의 상태는 유지한다. Git worktree, 풀의 기기와 저장소 공용 빌드는 삭제하지 않는다. 다음 배정에서는 호환되는 공용 빌드를 다시 설치하고, 호환 빌드가 없을 때만 새로 빌드한다.
...
네이티브 빌드는 느리지만 플린 앱 데이터와 로그인 상태는 worktree마다 달라야 한다. 따라서 같은 플랫폼과 native fingerprint의 빌드 결과만 공유하고 기기는 독점 배정한다. 반납할 때 플린 앱만 삭제하면 이전 작업의 앱 데이터를 정리하면서 Apple·Google 기기 로그인을 반복하지 않을 수 있다. 최대 5개 제한은 기기가 계속 늘어나는 일을 막고, 일반 종료 후 배정 유지는 같은 작업을 이어갈 수 있게 한다.
