...
- 컨트롤 안의 진행 표시는 원래 아이콘 자리와 문구 위치, 컨트롤 크기를 유지한다. 화면 전체가 대기의 대상인 자리에는 진행 표시만 화면 중앙에 둔다.
...
- 같은 자리의 아이콘과 진행 표시는 동일한 크기의 표시 영역을 사용한다. 문구 길이와 상태가 같으면 아이콘 교체 전후의 표시 영역과 간격도 같아야 한다. 크기, 간격과 글자 확대 시 적용 방식은 공통 역할별 기준이 소유한다.
- 누를 수 있는 영역과 보이는 아이콘의 크기를 구분한다. 간격을 줄여도 터치 영역은 유지하고, 그 영역이 이웃한 컨트롤의 입력을 가로채지 않게 한다. 같은 자리의 아이콘을 진행 표시로 바꾸는 일만으로 문구와 표시의 기준점이 움직이지 않게 한다.
