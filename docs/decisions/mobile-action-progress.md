...
- 이 진행 표시는 공통 `LoadingSpinner` 하나가 소유한다. 화면과 기능은 `ActivityIndicator`를 직접 부르지 않는다. 시스템 진행 표시를 유지하되, 크기와 정렬은 보조 문구, 컨트롤과 화면 중앙의 역할에 맞춘다. 화면마다 임의의 크기를 넘기지 않고 공통 기준으로 관리한다.
- 보조 문구 옆의 진행 표시는 문구보다 강조되지 않도록 크기를 맞춘다. 문구가 여러 줄이면 첫 줄의 세로 중앙에 맞춘다. 작은 진행 표시에 맞춰 문구의 글자 크기 확대를 막지 않는다.
- 컨트롤 안의 진행 표시는 원래 아이콘 자리와 문구 위치, 컨트롤 크기를 유지한다. 공용 `Button`은 예외로 HeroUI 줄 안 선행 슬롯을 사용하며, 진행 표시가 생기거나 앞 아이콘을 바꿀 때 문구 위치가 조금 이동할 수 있다. 버튼의 문구와 상자 크기는 유지한다. 화면 전체가 대기의 대상인 자리에는 진행 표시만 화면 중앙에 둔다.
- 메시지 아래 아이콘 줄처럼 아이콘 하나만 담은 작은 버튼 안에서는 그 자리의 아이콘과 같은 16px로 그린다. 버튼 크기가 붙박여 있으므로 글자 크기를 따라 자라지 않는다.
- 문구와 진행 표시 또는 재시도 아이콘의 배치에는 역할별로 정한 표시 영역 크기와 간격 값을 사용한다. 간격은 문구 영역 끝에서 표시 영역 시작까지 잰다. 터치 영역은 이 계산에 넣지 않으며, 화면별 눈대중으로 간격을 조정하지 않는다.
- 공용 `Button`의 줄 안 선행 슬롯을 제외하고, 같은 자리의 아이콘과 진행 표시는 동일한 크기의 표시 영역을 사용한다. 문구 길이와 상태가 같으면 아이콘 교체 전후의 표시 영역과 간격도 같아야 한다. 크기, 간격과 글자 확대 시 적용 방식은 공통 역할별 기준이 소유한다.
- 누를 수 있는 영역과 보이는 아이콘의 크기를 구분한다. 간격을 줄여도 터치 영역은 유지하고, 그 영역이 이웃한 컨트롤의 입력을 가로채지 않게 한다. 공용 `Button`의 줄 안 선행 슬롯을 제외하고, 아이콘을 진행 표시로 바꾸는 일만으로 문구와 표시의 기준점이 움직이지 않게 한다.
- 역할별 기준값은 실제로 보이는 크기와 무게감을 확인해 정한다. 정한 뒤에는 같은 역할에 같은 값을 적용하며, 시각 검증을 이유로 화면마다 값을 바꾸지 않는다. iOS와 Android의 기본 글자, 큰 글자와 여러 줄 문구에서 공통 기준의 적용을 확인한다.
...
- 자동 실행 작업이 1초 안에 끝나면 시각적인 진행 표시를 띄우지 않는다. 1초를 넘기면 작업 대상이 있던 영역을 유지한 채 진행 표시를 보여 준다. 주변 정보만으로 기다리는 대상을 알 수 없는 자리에는 구체적인 상태 문구도 둔다.
- 화면 전체를 대기에 쓰는 작업은 상태 문구를 두지 않는다. 앱 진입과 에피소드 대화 읽기가 그런 자리다. 화면 중앙에 진행 표시 하나만 남기고, 무엇을 기다리는지는 화면 읽기에만 이름으로 전한다.
- 화면 중앙에 홀로 서는 진행 표시는 36px로 그린다. 문구 옆에 서는 표시의 크기를 쓰지 않는다.
- 제목이나 머리글을 위에 보여 둔 채 그 아래 한 칸만 기다리는 자리는 화면 일부 로딩이다. 표현 돌아보기의 표현 읽기가 그렇다. 이 화면은 사용자 요청에 따라 본문 가운데 `LoadingSpinner`만 두고 보이는 상태 문구를 두지 않는다. 화면 읽기에는 표현을 불러오는 중임을 알린다.
- 표현 돌아보기는 공통 `LoadingSpinner`의 `standalone` 표시를 쓴다. 감싸는 요소에
  `progressbar` 역할, `busy` 상태와 `표현을 불러오는 중`이라는 접근성 이름을 준다.
  처음 목록을 읽는 동안에는 칭찬과 `기억해 둘 표현`을 보여 주지 않는다.
- 대화 기록의 `최근 대화` 조회도 보이는 상태 문구를 두지 않는다. 목록을 기다리는 영역 중앙에
  공통 단독 표시 크기의 스피너만 둔다. 소제목으로 기다리는 대상을 알 수 있으므로
  `대화를 불러오고 있어요`를 반복하지 않고, 로딩 이름은 화면 읽기에만 전한다.
- 로그인 상태 확인과 필수 프로필 확인은 하나의 진입 대기로 보고 1초 기준을 함께 적용한다. 단계가 바뀌어도 대기 시간을 다시 세지 않는다.
- 진입 확인이 끝나면 실제 상태에 맞는 화면을 열고, 실패하면 기존 오류와 다시 시도 경로를 사용한다. 이미 사용 중인 화면의 백그라운드 갱신을 화면 전체 로딩으로 바꾸지 않는다.
...
공용 `Button`은 문구 위치를 고정하기 위한 별도 배치 대신 HeroUI의 줄 안 슬롯을 유지한다. 사용자가 진행 중의 작은 문구 이동을 허용했으며, 버튼의 뜻과 상자 크기를 유지하면 된다. 다른 아이콘 전용 컨트롤과 상태 줄의 기준점은 바꾸지 않는다.
...
앱 진입에서는 아직 사용할 화면이 없으므로 화면 전체가 대기의 대상이다. 확인이 늦어질
때 중앙에 로딩을 표시하면 멈춘 빈 화면과 구분할 수 있다. 여기에 문구까지 두지 않는
이유는 그 글이 읽을 대상이 아니기 때문이다. 무엇을 기다리는지는 사용자가 방금 한
행동이 이미 말해 준다. 앱을 열었으니 앱이 열리는 중이고, 에피소드를 눌렀으니 그
대화를 여는 중이다. 화면 읽기에는 같은 사실을 이름으로 전하므로 잃는 것도 없다.
화면의 일부만 기다릴 때는 반대다. 화면의 나머지가 살아 있으므로 무엇이 기다리는
중인지 그 자리가 스스로 말해야 한다. 이것이 두 경우를 가르는 기준이다.
표현 돌아보기는 화면 제목과 화 정보가 남아 있고, 그 아래 표현 목록 자리에서만
기다린다. 사용자는 기다리는 대상을 알 수 있으므로 이 자리에는 문구 없이 진행
표시만 둔다.

화면 한가운데 홀로 서는 표시에 문구 옆 크기를 쓰면 빈 배경에서 먼지처럼 흩어져
보인다. iOS `ActivityIndicator`의 `large`가 36pt, Material `CircularProgressIndicator`의
기본이 40dp라 36px은 두 운영체제가 화면 중앙 로딩에 쓰는 범위 안에 있다.

채팅의 중지는 입력창에 한곳만 두어 질문, 다시 받기와 재시도 중 어떤 동작으로
시작했든 같은 자리에서 멈춘다.
...
이 색 규칙을 컴포넌트 하나가 담는 이유는 값을 빠뜨릴 자리를 없애기 위해서다. 규칙을 화면마다 적으면 플랫폼 분기와 색 조회가 자리마다 반복되고, 다음 자리를 만드는 사람이 색을 넘기지 않아도 화면은 그려진다. 크기도 역할별 공통 기준으로 관리한다. `small` 하나를 모든 자리에 적용하면 작은 보조 문구 옆에서 진행 표시가 지나치게 커질 수 있다. 시스템 모양은 유지하면서 문구와 컨트롤의 역할에 맞는 비율을 사용한다.

작은 재시도 아이콘을 넓은 터치 영역의 가운데에 놓으면, 터치 영역의 안쪽 여백이 문구와
아이콘 사이의 간격에 더해진다. 보이는 요소 사이의 간격과 누를 수 있는 영역을 구분하면
안내와 동작을 가까이 묶으면서도 누르기 쉬운 크기를 유지할 수 있다.

감싸는 요소와 문구는 이 컴포넌트에 넣지 않는다. 문구가 없는 자리가 있고 문구 스타일도 자리마다 달라서, 함께 묶으면 배치 옵션이 붙기 시작한다.
...
- 사용자가 방금 한 행동만으로는 무엇을 기다리는지 알 수 없는 화면 전체 대기가 생긴다.
...
- 시스템 진행 표시의 크기나 글자 확대 방식이 바뀌어 역할별 크기와 정렬이 맞지 않게 된다.
...
- 두 플랫폼에 같은 색을 쓰기: 한 값으로 끝나지만 목표가 각 운영체제의 진행 표시처럼 보이는 것이라 그 관례를 버리게 된다. 브랜드 표현이 플랫폼 관례보다 앞서야 할 때만 다시 검토한다.
- 화면 전체 로딩에 상태 문구를 함께 두기: 화면을 채우려고 놓는 글이 된다. 사용자가 방금 한 행동만으로는 무엇을 기다리는지 알 수 없는 진입 경로가 생길 때만 다시 검토한다.
- 화면 전체 로딩에서 진행 표시까지 없애기: 멈춘 빈 화면과 구분할 수 없다. 네이티브 시작 화면이 그 대기 시간을 온전히 덮을 수 있게 될 때만 다시 검토한다.
- 화면 전체 로딩에 스켈레톤 그리기: 앱 진입과 대화 읽기 모두 들어올 내용의 뼈대를 미리 그릴 만한 구조가 없다. 그 자리에 들어올 내용이 고정된 틀을 갖게 될 때 다시 검토한다.
- `LoadingSpinner`에 화면별 임의 크기를 넘기기: 역할별 공통 기준으로 비율을 유지한다. 공통 역할로 설명할 수 없는 새 진행 표시가 필요할 때 다시 검토한다.
- 모든 진행 표시를 같은 크기로 고정하기: 작은 보조 문구와 컨트롤의 크기 차이를 무시한다. 실제 화면에서 역할 간 크기 차이가 필요 없다는 근거가 생길 때 다시 검토한다.
- 문구와 아이콘을 가깝게 보이게 하려고 터치 영역을 줄이기: 시각적인 간격 문제를 누르기 어려운 문제로 바꾸지 않는다.
