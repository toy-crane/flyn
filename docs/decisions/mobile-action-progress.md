...
- 컨트롤 안의 진행 표시는 원래 아이콘 자리와 문구 위치, 컨트롤 크기를 유지한다. 화면 중앙의 진행 표시는 상태 문구와 하나의 묶음으로 배치한다.
...
- 앱 진입처럼 화면 전체를 사용할 수 없는 작업은 화면 중앙에 기본 스피너와 상태 문구를 표시한다. 로그인 상태 확인과 필수 프로필 확인은 하나의 진입 대기로 보고 1초 기준을 함께 적용한다. 단계가 바뀌어도 대기 시간을 다시 세지 않는다.
...
때 중앙에 로딩을 표시하면 멈춘 빈 화면과 구분할 수 있다. 채팅의 중지는 입력창에
한곳만 두어 질문, 다시 받기와 재시도 중 어떤 동작으로 시작했든 같은 자리에서 멈춘다.
