...
- React Native UI에서 누를 수 있는 것과 표면은 HeroUI Native 컴포넌트나 그것을 감싼 저장소 공용 컴포넌트로 만든다. 버튼은 공용 `Button`, 눌리지 않는 라벨 알약은 `Chip`, 누르는 작은 알약은 `Button`의 `sm` 크기, 글자만 있는 작은 링크는 `LinkButton`, 목록은 `ListGroup`과 항목 사이 `Separator`, 펼치지 않는 카드는 `Card`, 펼침 카드는 `Accordion`의 표면 변형, 구분선은 `Separator`, 입력의 오류 문구는 `FieldError`를 쓴다. HeroUI `Chip`은 눌림 반응과 비활성 표현이 없으므로 누르는 자리에 쓰지 않는다.
...
- 연속 기록의 날짜 칸: 색 단계로 칠한 정사각형 칸이라 `Button`의 모양이 맞지 않는다. `PressableFeedback`의 기본 축소 반응을 쓰고, 아직 오지 않은 날은 `isDisabled`로 잠근다. 달력의 이전·다음 달은 공용 아이콘 버튼의 44pt 크기 안에 작은 원을 그린다.
...
- 회차 Sheet의 현재 화 강조는 [모바일 스토리 탐색](mobile-story-browsing.md)이 정한
  상태 표현만 더한다. `ListGroup`과 공용 행의 구조, 기본 크기·여백·둥글기·누름 반응을
  유지하고 현재 화의 배경과 행동 문구에만 공통 의미 색과 굵기를 적용한다. 다른 목록의
  기본값이나 `global.css`의 재정의를 늘리지 않는다. 회차 삭제 메뉴는 HeroUI `Menu.Item`의
  `danger` 변형과 아이콘 슬롯을 사용한다.
...
- 카드, 목록 행, 칩, 인증 코드 칸의 기본 모양을 앱이 조정하기: 2026-09-13 시안에서 지금 모습(A), HeroUI 기본(B), 본문 행간만 재정의(C)를 비교했고 사용자가 가장 HeroUI다운 B를 골랐다. 이 계약에 명시한 현재 화의 상태 표현은 기본 크기나 모양 변경과 구분한다. 한국어 화면에서 기본값이 실제로 문제를 만들면 global.css 한 곳에서 재정의한다.
