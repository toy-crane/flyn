...
- 턴 상한은 코드가 소유하는 시스템 상수지만 에피소드는 **생성 시점 값을 복사해**
  갖는다. 상수를 바꿔도 이미 끝난 에피소드의 종료 사유가 흔들리면 안 된다.
- 늦게 도착하는 판정은 상태 컬럼이 아니라 `message_feedback`의 행으로 표현한다.
  행이 없음이 곧 판정 전이다.
...
- 어떤 테이블이 있고 무엇을 담는지는 이 계약이 정하지 않는다.
  `supabase/migrations/`의 스키마가 소유한다.
