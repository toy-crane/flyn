...
- 생성된 마이그레이션은 그대로 커밋하는 것을 기본으로 한다. 권한을 이유로 손으로 보정하지 않으며, DML과 선언형 diff가 표현하지 못하는 객체만 수동으로 보완한다.
- Supabase의 자동 노출 차단 기본값을 신뢰한다. 새 테이블과 함수는 명시적 GRANT 없이 Data API 역할에 닿지 않으므로, 테이블과 함수마다 REVOKE 상용구를 두지 않는다. 기본 권한이 남기는 REFERENCES, TRIGGER, TRUNCATE, MAINTAIN은 Data API로 닿을 수 없으므로 받아들인다.
...
권한 수동 보정이 실제로 지우던 것은 Data API로 닿을 수 없는 네 권한뿐이고, 행 접근은 RLS가 막는다. 테이블마다 반복되는 손 보정과 별도 권한 확인은 그 실익보다 비용이 크다. 그래서 Supabase가 권장하는 기본 방법을 기본값으로 삼고, 생성된 마이그레이션을 그대로 쓴다.
...
- 테이블과 함수마다 기본 권한을 수동 REVOKE로 회수: 생성된 마이그레이션마다 손 보정과 별도 권한 확인이 필요해진다. Data API로 닿는 권한은 자동 노출 차단 기본값이 이미 막고 행 접근은 RLS가 막으므로, 임의 SQL 실행 경로가 생길 때만 재검토한다.
