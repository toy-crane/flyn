...
| --- | --- | --- |
| 연속 기록 한 줄(`12일 연속`) | `Heading` `h2` | 홈과 연속 기록 화면의 맨 위에서 큰 숫자로 선다 |
...
| 카드 제목, 영어 문장, 회차 날짜, 이번 주 카드의 주차와 달력의 달 | `Heading` `h6` | 펼침 카드의 제목은 트리거 안에 있어 헤더로 따로 읽히지 않을 수 있다. 펼침 카드는 펼침 상태를 가진 버튼으로, 회차 카드 본문은 Sheet를 여는 버튼으로 읽힌다. 배울 표현의 고친 문장도 영어 문장이라 헤더로 읽힌다 |
...
| 본문, 말풍선, 대사 뜻, 장면 서술, 안내 문단, 달력 아래의 합계 줄 | `Paragraph` `body` | 합계 줄의 이름은 `color="muted"`, 값은 `weight="semibold"` |
| 작은 본문, 표현 노트와 표현 돌아보기의 뜻, 화 결과, 상황 줄, 오류 문구, 날짜 칸의 숫자와 툴팁 | `Paragraph` `body-sm` | 오류는 색만 `text-danger`. 입력에 붙는 오류는 `FieldError`가 그린다. 날짜 숫자는 `weight="medium"`이고 아직 오지 않은 날만 보통 굵기다 |
| 캡션, 메타, 출처, 채널 라벨, 인물 이름표, 달력의 요일 | `Paragraph` `body-xs` | 강조는 `weight`, 색은 className |
...
- 연속 기록의 날짜 숫자와 요일은 1.5배를 상한으로 둔다. 한 줄에 7칸을 두는 격자라 가장 좁은 375pt 화면의 칸이 약 38pt이고, 상한이 없으면 최대 크기에서 숫자가 칸에 잘린다. 같은 날짜와 횟수는 칸의 접근성 이름과 툴팁이 상한 없이 전한다. 2026-09-15 iOS `accessibility-extra-extra-extra-large`와 Android 200%로 재시작해 칸, 카드 머리, 합계 줄과 툴팁이 잘리지 않음을 확인했다.
...
앱을 연 채 바꾸는 경우를 지원하지 않는 이유는 그 경우만 빼면 상한 없이도 화면이 그대로 통과하기 때문이다. React Native는 글자 크기와 줄 높이를 같은 배율로 키우고, HeroUI Native도 상한 없이 이 기본값을 따르므로 앱을 완전히 닫고 다시 시작한 화면은 최대 크기에서도 손질 없이 그려진다. iOS와 Android의 홈, 탐색, 대화, 종료, 표현 돌아보기, 표현 노트와 설정에서 기본 크기와 최대 크기로 재시작한 화면을 확인했다. 반면 앱을 연 채 바꾸면 React Native가 글자만 새 크기로 그리고 텍스트 상자는 이전 크기에 남긴다. 이는 앱 코드로 고칠 수 없는 프레임워크 동작이고, 화면마다 뷰를 강제로 다시 만드는 처치가 늘어나는 원인이었다. WCAG 1.4.4는 200% 확대에서 내용과 기능이 남아 있기를 요구할 뿐 실시간 반영을 요구하지 않는다.
