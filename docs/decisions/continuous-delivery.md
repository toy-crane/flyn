...
- `fingerprint.yml`은 PR마다 iOS fingerprint를 계산하고, `fingerprint-labels.yml`은 `main`의 코드로 라벨만 쓴다. 둘은 `ci.yml`과 따로 둔다. `deployment-access.yml`은 수동으로 운영 연결만 읽어 보는 검사다.
...
- 새 DB 마이그레이션이나 Edge Function 운영 설정이 있으면 `flyn-production-review` 환경에서 별도 승인을 받아야 한다. 그 외 변경은 `flyn-production-automatic` 환경에서 계속 자동 배포한다. 운영 배포의 사람 승인은 이 환경 승인 하나다. 승인 화면에는 SQL이 아니라 실행만 보이므로 SQL은 PR 리뷰 때 읽는다.
- 승인 판정은 `plan` 잡이 한다. 미적용 마이그레이션은 Supabase dry-run으로, Edge 함수 설정 변경은 기준 커밋과의 diff로 본다. 이를 위해 `plan`에만 읽기용 Supabase 접근 토큰을 준다.
...
- 서비스별 건너뛰기는 그 서비스가 알려 준 커밋을 기준으로 diff한다. 알려 주지 못하면 GitHub 실행 이력의 기준 커밋을 쓴다. 앞선 실행이 API에서 실패했어도 이미 올라간 Edge를 다시 배포하거나 다시 승인받지 않는다. 승인한 함수 설정은 함수 소스와 함께 배포하고 원격 설정·소스를 확인한 뒤 성공으로 본다.
...
- 검토한 SQL의 SHA-256 목록을 저장소에 두기: 승인 환경과 같은 일을 두 번 한다.
