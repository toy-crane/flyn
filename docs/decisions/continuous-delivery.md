...
- iOS `Flyn Internal`과 `internal` 채널의 자동 모바일 배포 결과는 EAS Workflows의 공식 Slack 연동과 Slack Incoming Webhooks로 알린다. 별도 알림 서버와 배포 승인 경로를 만들지 않는다.
- 호환 빌드를 재사용한 OTA와 기존 빌드를 제출한 뒤 게시한 OTA는 `OTA 게시 완료`로 알린다. 후자는 기존 빌드 제출 사실도 포함한다. 새 빌드 제출 뒤 설치 가능 여부를 확인한 경우만 `TestFlight 설치 가능`으로 알린다. 빌드·제출 성공만으로 설치 가능이라 하거나 OTA 게시를 모든 기기의 적용 완료로 표현하지 않는다.
- 준비·빌드·제출·설치 가능 확인·OTA 실패는 실패한 단계와 확인하지 못한 결과를 알린다. 선택하지 않은 경로의 건너뛰기는 실패가 아니다. Apple 처리 결과를 확인하지 못했다면 단정하지 않는다.
- 모든 알림에는 프로젝트, 플랫폼, 배포 커밋과 EAS 실행 링크를 넣는다. 빌드 관련 알림에는 확인 가능한 앱 버전과 빌드 번호를 넣고, OTA에는 채널과 업데이트 링크를 넣는다. 아직 없는 값이나 링크를 만들지 않는다. 단계 로그의 직접 링크가 없으면 실행에서 해당 단계를 찾게 한다.
- Webhook URL은 서버용 비밀값이다. 저장소, 앱 번들, 로그와 알림에 노출하지 않는다.

다음은 알림 명세에서 정한 변경 가능한 기본값이다.
...
- 강제 업데이트는 [모바일 업데이트 경험](mobile-update-experience.md)을 따른다.
