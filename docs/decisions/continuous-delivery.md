...
- `main`에 새 커밋이 들어오면 `main`을 대상으로 열린 PR의 이전 fingerprint 라벨을 지운다. PR은 `main`만 대상으로 열며, 다른 base 브랜치를 쓰기 시작하면 라벨 워크플로의 트리거를 다시 넓힌다. 다음 PR 계산이 끝나기 전에는 호환성을 표시하지 않는다. 외부 fork 결과는 신뢰한 비교값으로 게시하지 않고 `Fingerprint:error`로 표시한다. 같은 저장소의 계산 코드 변경도 리뷰 대상이며 라벨을 배포 승인이나 보안 증명으로 사용하지 않는다.
...
- 배포 상태를 저장소 안의 파일로 기록하지 않는다. 배포 대상 판정의 기준 커밋은 같은 워크플로의 마지막 성공한 `main` 실행에서 읽고, 이미 올라간 것과 진행 중인 것은 각 서비스에 커밋과 요청 ID로 묻는다. 서비스가 모르는 성공 기록은 신뢰하지 않는다.
- 검사와 배포는 한 워크플로 실행 안에서 `needs`로 잇는다. 배포 잡이 다른 실행의 검사 결과를 조회해서 기다리지 않는다.
- 새 DB 마이그레이션이나 Edge Function 운영 설정이 있으면 `flyn-production-review` 환경에서 별도 승인을 받아야 한다. 그 외 변경은 `flyn-production-automatic` 환경에서 계속 자동 배포한다.
- 2026-09-10: 운영 배포의 사람 승인은 이 환경 승인 하나만 둔다. 검토한 SQL의 SHA-256을 저장소에 적어 두던 목록은 없앤다. 승인 화면에는 SQL이 아니라 실행만 보이므로, SQL은 PR 리뷰 시점에 읽는다. 승인하는 사람이 둘 이상이 되면 이 결정을 다시 본다.
...
- 저장소 안에 서명한 배포 상태 파일 두기: 2026-09-10까지 썼다. 서비스와 GitHub 실행 이력이 같은 사실을 이미 알고 있어 기록이 둘로 늘고, 서명 키와 검증 코드가 따라온다.
...
구현 시 원본과 설치 버전을 다시 확인한다. 스펙의 완료 조건은 [내부 테스트 자동 배포](../specs/continuous-delivery/spec.md)가 정하고, 워크플로 구조와 배포 상태 조회 방식은 [CI 워크플로 단순화](../specs/ci-workflow-simplification/spec.md)가 대체한다.
