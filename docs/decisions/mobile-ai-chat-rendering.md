...
- 내용과 표시 조건이 같은 지난 메시지 본문은 다른 답변의 시작과 완료만으로 다시
  ...
  메뉴를 감싸는 영역을 유지한다.
- 말풍선 바깥 옆에 작은 동작 버튼이 설 수 있다. 말풍선을 감싸는 자리를 화면이
  넘겨주고, 그 곁에 무엇을 두는지는 패널이 들여다보지 않는다. 곁에 선 것이
  바뀌어도 목록은 그 행을 다시 만들지 않는다. 지금 이 자리를 쓰는 것은
  [표현 노트](../specs/saved-expressions/spec.md)의 책갈피다.
