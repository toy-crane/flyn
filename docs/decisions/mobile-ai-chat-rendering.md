...
- 말풍선 하나에 걸리는 작은 동작 줄이 그 아래에 설 수 있다. 말풍선을 감싸는
  ...
  이 자리를 쓰는 것은 [표현 노트](../specs/saved-expressions/spec.md)의 복사와
  책갈피다.
- 방금 한 일을 알리는 짧은 문구는 화면 위쪽에 잠시 뜬다. 누를 동작을 두지 않고,
  떠 있는 동안에도 대화를 이어 갈 수 있다. 이 문구는 채팅 패널이 아니라 HeroUI
  Native의 토스트가 그리며, 자리와 안전 영역, 키보드 회피, 겹쳐 쌓기를 그쪽이
  소유한다. 화면 아래쪽은 입력창, 최신 메시지 버튼과 키보드가 차례로 쓰는데
  토스트는 화면 전체를 덮는 층에 그려져 그 컨트롤들의 높이를 알 수 없다.
