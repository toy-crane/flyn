...
- 모델 호출 시도마다 request ID, model, duration, finish reason, 결과와 token usage를
  구조화 로그 한 건으로 남긴다.
...
- 모델은 AI SDK와 Gateway를 통하며 생산 모델은 코드에 고정한다.
...
동시에 도착하면 모델 호출이 중복될 수 있다. 긴 채팅은 context 한도를 넘을 수
