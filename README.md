...
10. 홈 본문이 비어 있고, 오른쪽 프로필로 설정을 열었다 돌아올 수 있는지
    확인합니다. 하단 탭이 **홈 / 탐색 / 스토리** 셋인지도 봅니다.
11. **탐색**에서 스토리를 하나 골라 상세를 엽니다. 모든 화의 제목과 상황 설명이
...
11번부터 13번까지를 실행하려면 [AI 채팅 API](#ai-채팅-api)가 떠 있어야 합니다.
...
| `다른 이메일 사용` | 이메일 수정 버튼 |
| `로그인 상태 확인 중` | 앱을 열고 로그인 상태를 확인하는 동안의 진행 표시. 화면에 문구는 나오지 않습니다 |
| `프로필 확인 중` | 이어서 프로필을 확인하는 동안의 진행 표시. 화면에 문구는 나오지 않습니다 |
...
| `이어서 하기` | 대화 기록의 회차 카드에서 진행하다 만 화를 다시 여는 버튼 |
| `탐색에서 스토리 고르기` | 대화가 없는 스토리 탭의 버튼 |
| `<제목>, <한 줄 소개>` | 탐색과 스토리 탭의 목록 행. 소개가 없으면 제목만입니다 |
| `<시작 시각>, 대화 기록 펼치기` | 회차 카드를 펼치고 접는 자리 |
...
| `대화를 불러오고 있어요.` | 에피소드 대화를 1초 넘게 읽을 때 본문 가운데 진행 표시. 화면에 문구는 나오지 않습니다 |
...
- 홈: `home-scroll`. 본문은 아직 비어 있습니다
- 탐색 탭: `browse-scroll`, `browse-row-<스토리 id>`, `browse-unavailable`
- 스토리 탭: `recent-scroll`, `recent-row-<스토리 id>`, `recent-empty`, `recent-unavailable`
- 스토리 상세: `story-detail-scroll`, `story-episode-<화 번호>`, `story-start`, `story-detail-unavailable`
- 대화 기록: `story-records-scroll`, `story-records-empty`, `story-records-unavailable`
...
대화가 쌓이면 같은 이름이 여러 개가 되므로, 답변 하나를 확인할 때는 해당 물어보기 시트를 열고 본문으로 대상을 구분합니다.
...
`bun run dev:remove`는 이 폴더에 배정한 기기에서 플린 앱만 삭제한 뒤 저장소 기기 풀로 돌려놓고 포트를 반납합니다. 기기에 로그인한 Apple·Google 계정과 설정은 유지합니다. 앱 삭제가 실패하면 반납을 중단합니다.
Git worktree, 풀의 기기와 저장소 공용 빌드는 지우지 않습니다. 다음 배정에서 필요한 공용 빌드를 다시 설치합니다.

기기는 플랫폼별 최대 5개까지 필요할 때 만듭니다. 모두 배정 중이면 `bun run dev:status`로 확인하고 사용하지 않는 worktree에서 `bun run dev:remove`를 실행하세요. `dev:stop`은 기존 배정을 유지합니다.
...
`GET /health`는 인증 없이 서버 상태만 알려 주고, `POST /ai/episode`와 `POST /ai/episode/ask`는 로그인한 사용자의 access token을 요구합니다.
...
3. **생성된 마이그레이션을 그대로 두고 검토 에이전트가 읽게 합니다.**
   파괴적인 변경, 권한과 RLS, 뷰와 함수의 보안, diff가 놓친 객체, 잠금과 실행 순서를 확인합니다.
   권한 때문에 손으로 고치지 않습니다. 손으로 보완하는 것은 DML과 선언형 diff가
   표현하지 못하는 객체뿐입니다. `storage` 정책, `auth` 스키마의 트리거가 그 예입니다.
