...
표현 확인 평가는 기존 21개, 상황 표현 10개와 여러 오류 1개를 각각 세 번 확인한다. 표기와 채팅
말투를 그대로 두는지, 철자와 문법 오류는 고치는지, 실제 오류와 상황 표현을
구분하는지 검사한다. 뜻, 태도, 수량과 조건도 보존해야 한다. 평가는 운영과 같은
판정과 한 번의 출력 복구를 거친 뒤 실제 서버 검사로 결과를 확인한다. 동시 호출은
두 개로 제한한다. 하나라도 실패하거나 답을 받지 못하면 명령이 실패한다. 모델,
실행 시각, 프롬프트, 출력 전문과 실패 이유는 `results`에 기록한다.
...
- [요청 행동 보존을 보완한 중간 결과](results/correction-candidate-1789474249764.json): 기존 21문장과
  상황 표현 10문장을 합친 31문장 × 3회, 93개 모두 통과. 실제 오류와 표현 제안,
  뜻과 태도 보존, 문맥별 판정, 한국어·한영 혼합과 불분명한 입력을 함께 확인했다.
- [프롬프트 정리 전 실패 결과](results/correction-candidate-1789475400004.json): 32문장 × 3회,
  96개 중 94개 통과. 오류별 분류, 독립된 여러 오류와 제안문을 반복하는 예문 검사까지
  추가했다. `I dont wants it`을 `I doesnt want it`으로 바꾼 오답 1개와 서버의 표기 검사에
  거절된 응답 1개가 남았다. 앞선 93/93 결과를 현재 기준의 전체 통과로 보지 않는다.
  이 보완 과정의 앞선 실행도 각각 94/96, 93/96, 95/96이었다.
- [프롬프트와 출력 순서를 정리한 최종 결과](results/correction-candidate-1789475800984.json):
  같은 모델로 96/96 통과. 판정, 완성 문장, 대응 조각, 다른 예문 순서로 프롬프트를
  정리하고 스키마도 완성 문장을 먼저 생성하도록 바꿨다. 현재 평가 기준으로 저장된
  96건을 다시 검사하는 자동 테스트를 추가했다. 이번 통과는 고정 사례의 결과이며
  모델의 모든 응답이 항상 정확하다는 뜻은 아니다.
...
고정 입력과 이전 프롬프트는 [평가 입력](fixtures/role-ownership-scenarios.json)에 둔다.
과거 실험 보고서와 전체 응답 기록은 삭제했다. 평가에 필요한 입력과 실제 오답 예시는
이 폴더에서 관리한다.

2026-09-14 Luna 실험에서 다음을 확인했다.

- 결말과 기록을 빼고 대사만 생성해도 일정의 주인을 혼동했다. 작업을 나누는
  것만으로 해결된다고 볼 근거는 없다.
- 일정의 주인만 읽는 요청은 40회 모두 맞았지만, 그 사실을 전달한 대사 생성에는
  오류가 남았다. 초안을 한 번 더 고치는 방식도 기존 오류를 남기고 새 오류를 만들었다.
- 네 규칙 후보는 일정 오류를 65회 중 12회에서 2회로 줄였다. 상대 전환, 질문 답변,
  결말과 발화 수 문제는 남아 품질 기준을 통과하지 못했다. 운영에 적용하지 않은 이유다.

같은 카페 상황의 변형을 반복한 결과이며, 다른 이야기나 모델의 성능을 뜻하지 않는다.
상세 설계와 원문은 Git 커밋 `8802d5c`의
`docs/specs/episode-prompt-load-evaluation/`에서 확인할 수 있다.
