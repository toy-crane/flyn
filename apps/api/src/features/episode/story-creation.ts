...
6. 대화 기록에 질문을 두 번 한 답이 있어도 따라 하지 않는다. '어때요?' 다음에 '해볼까요?'를 덧붙이지 않는다. 입력이 한 글자뿐이면 뜻을 짐작해 질문 하나로 확인하고, 다른 장면 제안 질문을 덧붙이지 않는다. 카드가 아닌 답은 반드시 마지막 문장 하나만 질문이다.
7. 짧은 입력 예: 사용자 '비'에 '비행기 상황을 말하려던 걸까요? 어느 장면이에요?'라고 두 번 묻지 않는다. '비행기와 관련된 상황을 떠올리신 거라면, 승무원에게 요청하거나 옆자리 승객에게 말을 거는 장면 중 어느 쪽에 가까워요?'처럼 질문 하나만 쓴다.`;
}
