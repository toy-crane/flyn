 ...
 * 곁의 글자를 따라 함께 커진다.
 *
 * 문구가 서는 역할의 줄 높이는 곁의 글자가 쓰는 타이포그래피 대응표의 값이다.
 * `control`은 `body`(28), `screen`은 `body-sm`(24), `supporting`은 `body-xs`(20)다.
 */
const ROLES = {
  /** 메시지 아래 아이콘 줄처럼 28px 버튼 안. 그 자리의 아이콘과 같은 크기다. */
  compactControl: { fixed: true, gap: 6, indicator: 16, lineHeight: 16 },
  control: { fixed: true, gap: 8, indicator: 20, lineHeight: 28 },
  screen: { fixed: false, gap: 8, indicator: 20, lineHeight: 24 },
  /** 문구 없이 화면 가운데 홀로 서는 표시. 곁에 따라 자랄 글자가 없다. */
  standalone: { fixed: true, gap: 0, indicator: 36, lineHeight: 36 },
  supporting: { fixed: false, gap: 6, indicator: 14, lineHeight: 20 },
} as const;
