...
/** 전송이 실제로 실어 보내는 몸통. */
/**
 * `storyPlayId`를 `null`로 주면 아직 회차가 없는 새 대화다. 생략과 구분해야 해서
 * `undefined`가 아니라 `null`을 쓴다. 기본값은 생략을 이어가기로 읽는다.
 */
function bodyOf(
  messages: UIMessage[],
  trigger: "regenerate-message" | "submit-message",
  storyPlay: string | null = STORY_PLAY_ID
): Record<string, unknown> {
  const storyPlayId = storyPlay ?? undefined;
...
test("이어가는 요청은 회차를 싣고 스토리는 싣지 않는다", () => {
  ...
  expect(body.storyPlayId).toBe(STORY_PLAY_ID);
...
test("새 대화는 스토리만 싣는다", () => {
  ...
  expect(body.storyPlayId).toBeUndefined();
});
