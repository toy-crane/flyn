...
--
-- 차단 주장("아무것도 안 바뀌었다")은 정책이 통째로 사라져 아무도 못 쓰게 돼도 참이다.
-- 그래서 소유자가 자기 행을 쓸 수 있다는 양성 대조를 함께 둔다 — 둘이 같이 있어야
-- "남의 행은 막고 내 행은 되는" 상태를 고정한다.
...
-- 양성 대조: UPDATE 정책이 사라지면 여기서 걸린다.
update public.scratch_notes set body = 'bob edit';
select is(
  (select count(*)::int from public.scratch_notes where body = 'bob edit'),
  1,
  'bob은 자기 행을 수정할 수 있다'
);
