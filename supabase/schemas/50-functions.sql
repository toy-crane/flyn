...
--
-- 스토리 하나에는 네 테이블의 행이 함께 들어간다. 앱이 이것을 네 번의 요청으로
-- 나누면 중간에 끊긴 자리마다 절반짜리 스토리가 남고, 스펙이 요구하는 "실패한
-- 시도는 아무것도 남기지 않는다"가 깨진다. 함수 하나가 도맡으면 그 넷이 한
-- 문장이라 중간이 없다. 그래서 네 테이블의 insert 권한은 아무에게도 주지 않고
-- 저장하는 길을 이것 하나로 둔다.
--
-- `security definer`인 이유는 그 권한을 함수만 갖기 위해서다. RLS가 걸리지
-- 않으므로 주인이 부르는 사람인지도 여기서 답한다. 요청이 주인을 말할 자리는
-- 없고, `auth.uid()`가 그 자리를 대신한다.
--
-- 화의 인물은 이름으로 받아 스토리 안에서 찾는다. 카드에 없는 이름이 오면
-- 외래키가 막고 스토리 전체가 없던 일이 된다.
...
comment on function public.create_story(jsonb) is
  'Saves one story the caller made, with its people and episodes, in a single statement. Returns the new story and its first episode.';
