...
(
  ...
  $content$💳$content$,
  $content$다음 날 아침, 어제 그 카페 계산대다.
아이스 아메리카노를 시키고 카드를 댔는데 두 번이나 거절됐다.
지갑 속 현금은 눈으로 봐도 조금 모자란다.
Mia: Oh, hi again. Hmm, it still says declined. Do you have another card?
...
- Mia는 어제 사용자의 주문을 처리한 그 직원이다. 방법을 같이 찾아 주려 하지만 외상은 규정상 해 줄 수 없다.
...
(
  ...
  $content$🪑$content$,
  $content$오후의 카페, 늘 앉던 창가 자리 앞이다.
전화를 받느라 오 분쯤 나갔다 온 사이 누가 그 자리에 앉아 노트북을 펴 두었다.
내 가방은 옆 테이블로 옮겨져 있다.
Owen: Oh, is this your bag? Sorry, the table looked empty.
Mia: Hey, you're back. Everything okay over there?$content$,
  $content$상황:
...
(
  ...
  $content$🥤$content$,
  $content$한산한 오후의 카페 카운터 앞이다.
Mia가 메뉴판에 없는 분홍빛 음료를 잔에 담아 내밀었다.
한 모금 마셔 보니 자몽 향은 좋은데 뒷맛이 꽤 쓰고 단맛이 따로 논다.
Mia: Try this. I made it myself. The manager tastes it on Friday, so be honest, okay?$content$,
  $content$상황:
...
(
  ...
  $content$👋$content$,
  $content$저녁 여덟 시, 문 닫기 십 분 전의 카페다.
메뉴판 맨 위에 지난주 시음했던 그 음료가 새 메뉴로 붙어 있다.
카운터 옆 종이 상자 위에는 Mia의 앞치마가 개어져 있다.
Mia: Oh, you came. Today is my last shift here. They asked me to run the new branch across town.
Mia: Iced americano, right? I got it wrong the first time we met.$content$,
  $content$상황:
...
(
  ...
  $content$🏨$content$,
  $content$밤 열 시, 낯선 도시의 호텔 프런트다.
긴 비행을 마치고 캐리어를 끌고 와 이름을 말했다.
휴대폰에는 예약 확인 메일이 예약 번호까지 또렷하게 떠 있다.
...
(
  ...
  $content$📊$content$,
  $content$다음 날 아침, 현지 사무소의 회의실 앞이다.
일주일을 준비한 한 시간짜리 발표 자료가 노트북에 들어 있다.
회의 시작까지 이십 분 남았다.
...
(
  ...
  $content$🍽️$content$,
  $content$저녁, Daniel의 단골 식당이다.
테이블에는 빈 접시가 쌓였고 방금 큰 접시가 하나 더 나왔다.
배는 이미 가득 찼고 내일 아침에는 일찍부터 일정이 있다.
...
(
  ...
  $content$🚕$content$,
  $content$출장 나흘째 아침, 호텔 로비다.
삼십 분 전에 불러 둔 택시가 아직 오지 않는다.
시내 건너편 거래처까지 가야 하고, 회의 시작까지 사십 분 남았다.
...
(
  ...
  $content$🧳$content$,
  $content$출장 마지막 날 아침, 캐리어를 끌고 내려온 호텔 로비다.
항공사가 비행기를 두 시간 앞당겨 Daniel과 하기로 한 점심을 취소했다.
공항 버스는 십오 분 뒤에 떠난다.
Daniel: I got your message. Forget lunch, I had to come anyway. We have about fifteen minutes, right?
Anna: Your bill is ready whenever you are. I'll keep it short.$content$,
  $content$상황:
...
- Daniel은 일주일 동안 사용자와 붙어 다녔고 이 배웅을 위해 아침 일정을 비웠다. 손에 든 작은 선물을 언제 줄지 재고 있다. 사용자가 마음을 전하면 선물을 내밀고, 일 이야기만 하면 가방을 든 채 끝난다.
...
(
  ...
  $content$🧊$content$,
  $content$이사 온 다음 날 아침, 함께 쓰는 집의 부엌이다.
장 봐 온 것을 넣으려고 냉장고를 열자 어제 비워 두기로 한 아래 칸까지 Jamie의 것으로 빼곡하다.
손에 든 장바구니는 여전히 무겁다.
Jamie: Morning! I did some grocery shopping last night. Just push my stuff aside if you need space.$content$,
  $content$상황:
...
(
  ...
  $content$🌙$content$,
  $content$새벽 두 시, 함께 쓰는 집의 부엌이다.
벽 너머 Jamie의 통화 소리에 사흘째 잠을 설쳤다.
물을 마시러 나오자 헤드셋을 목에 건 Jamie가 방문을 열고 나왔다.
...
(
  ...
  $content$🧾$content$,
  $content$저녁의 부엌 식탁이다.
Jamie가 보낸 이번 주 정산표가 폰에 떠 있다.
화장지와 세제 옆에 내가 쓰지 않는 원두와 방향제까지 전부 반반으로 적혀 있다.
Jamie: I just sent you the list for this week. I split everything fifty-fifty, easy, right?$content$,
  $content$상황:
...
(
  ...
  $content$🛋️$content$,
  $content$저녁, 현관문을 열고 들어선 거실이다.
Jamie의 친구 Noah가 사흘째 소파에 이불을 펴 두고 지낸다.
오늘은 욕실에 세면도구까지 늘어놓았다.
...
(
  ...
  $content$📝$content$,
  $content$함께 산 지 꼭 한 달째 되는 저녁, 부엌 식탁이다.
식탁에는 Jamie가 시킨 피자와 빈 종이 한 장, 펜 두 자루가 놓여 있다.
종이에는 아직 아무것도 적혀 있지 않다.
...
(
  ...
  $content$🗂️$content$,
  $content$월요일 아침 아홉 시, 영어로 일하는 회사의 첫 출근 날이다.
노트북을 펴자마자 옆자리에서 의자가 이쪽으로 돌았다.
인수인계 문서는 아직 받지 못했다.
Dan: Morning! You're the new one, right? I'm Dan.
Dan: Quick question. You're taking over the weekly numbers report, right? It goes out at eleven.$content$,
  $content$상황:
...
(
  ...
  $content$📅$content$,
  $content$화요일 오전, 첫 주간 회의의 마지막 순서다.
화면의 일정표에는 내 이름 옆에 수요일 마감인 일이 붙어 있다.
아직 그 자료에 접근할 권한도 받지 못했고, 지금 말하지 않으면 마감은 그대로 굳는다.
Grace: Okay, that's everyone. Anything else before we wrap up?
Dan: I'm good.$content$,
  $content$상황:
...
(
  ...
  $content$📊$content$,
  $content$수요일 오후, 사무실 내 자리다.
오전에 보낸 주간 보고서의 숫자가 틀렸다는 답장이 왔다.
확인해 보니 Dan이 건네준 원본 파일부터 지난주 숫자였고, Dan은 지금 회의 중이다.
Grace: Hey, the numbers in today's report don't match the system. Did you change something?$content$,
  $content$상황:
...
(
  ...
  $content$🙋$content$,
  $content$목요일 오후 다섯 시 반, 사무실이다.
내일 아침 회의에 낼 자료가 아직 반도 안 됐다.
시스템에서 데이터를 뽑는 단계에서 반나절째 막혀 있고, Grace는 이미 퇴근했다.
Dan: I'm heading out soon. You okay? You've been staring at that screen for a while.$content$,
  $content$상황:
...
(
  ...
  $content$🗣️$content$,
  $content$금요일 오후, 한 주를 정리하는 짧은 회의다.
화면의 다음 주 계획에 보고서 항목이 이번 주와 똑같이 적혀 있다.
수요일에 숫자가 꼬였던 그 방식 그대로다.
Grace: So, same process for the report next week. Unless anyone sees a problem?$content$,
  $content$상황:
...
(
  ...
  $content$🌙$content$,
  $content$아침의 아파트 복도, 엘리베이터 앞이다.
이사 온 지 일주일, 밤마다 천장에서 나는 소리에 사흘째 잠을 설쳤다.
분리수거를 하러 나왔다가 윗집 사람과 처음 마주쳤다.
Nora: Oh, hi! You must be the new neighbor in 302. I'm Nora, right above you.$content$,
  $content$상황:
...
- Nora는 아랫집 사람을 오늘 처음 만난다. 소리 이야기를 들으면 놀라고 진심으로 미안해한다.
...
(
  ...
  $content$📦$content$,
  $content$저녁, 1층 관리실 앞이다.
퇴근길에 배송 완료 문자를 받았는데 302호 문 앞에는 아무것도 없다.
새로 산 스탠드 조명이 든 상자다.
Frank: Evening. What can I do for you?$content$,
  $content$상황:
...
(
  ...
  $content$🧺$content$,
  $content$일요일 오후의 지하 세탁실이다.
전화를 받다가 이십 분 늦게 내려왔더니 돌려 둔 빨래가 꺼내져 테이블 위에 축축한 채로 얹혀 있다.
하나뿐인 멀쩡한 세탁기는 벌써 다른 빨래를 돌리고 있다.
Nora: Oh... hey. I was going to tell you about that.$content$,
  $content$상황:
...
(
  ...
  $content$🧾$content$,
  $content$저녁의 관리실이다.
우편함에서 꺼낸 첫 관리비 고지서에 이사 오기 전 달의 수도 요금까지 302호 앞으로 붙어 있다.
고지서를 들고 내려와 Frank에게 건넸다.
Frank: Let me see. Hmm, it looks right to me. Every charge under 302 goes to 302.$content$,
  $content$상황:
...
(
  ...
  $content$🍪$content$,
  $content$저녁 아홉 시, 302호 문 앞이다.
초인종이 울려 문을 열자 Nora가 서 있다.
손에는 쿠키가 든 작은 상자가 들려 있다.
Nora: Hi. These are for you. And... I also came to ask a favor, if that's okay.$content$,
  $content$상황:
