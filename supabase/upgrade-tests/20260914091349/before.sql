...
VALUES ('81111111-1111-4111-8111-111111111111', 'f1111111-1111-4111-8111-111111111111',
 'dddddddd-dddd-4ddd-8ddd-dddddddddddd', 'user', '[{"type":"text","text":"Hello"}]', '2026-01-01 00:02:00+00');
INSERT INTO public.language_levels(user_id, level, observed_at)
VALUES ('dddddddd-dddd-4ddd-8ddd-dddddddddddd', '지난 일을 영어로 설명한다', '2026-01-01 00:12:00+00');
INSERT INTO public.retired_usernames(username, retired_by, retired_at, protected_until)
VALUES ('retired_example', 'dddddddd-dddd-4ddd-8ddd-dddddddddddd', '2026-01-01 00:00:00+00', '2026-02-01 00:00:00+00');
CREATE SCHEMA ci_timestamps;
CREATE TABLE ci_timestamps.profiles AS SELECT * FROM public.profiles;
CREATE TABLE ci_timestamps.retired_usernames AS SELECT * FROM public.retired_usernames;
CREATE TABLE ci_timestamps.stories AS SELECT * FROM public.stories;
CREATE TABLE ci_timestamps.characters AS SELECT * FROM public.characters;
CREATE TABLE ci_timestamps.episodes AS SELECT * FROM public.episodes;
CREATE TABLE ci_timestamps.episode_characters AS SELECT * FROM public.episode_characters;
CREATE TABLE ci_timestamps.story_plays AS SELECT * FROM public.story_plays;
CREATE TABLE ci_timestamps.episode_plays AS SELECT * FROM public.episode_plays;
CREATE TABLE ci_timestamps.episode_messages AS SELECT * FROM public.episode_messages;
CREATE TABLE ci_timestamps.language_levels AS SELECT * FROM public.language_levels;
CREATE TABLE ci_timestamps.app_version_policies AS SELECT * FROM public.app_version_policies;
