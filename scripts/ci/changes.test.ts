...
const none = { affected: [], edgeConfiguration: false, paths: [] };
...
test("운영 함수 설정이 바뀌면 소스가 그대로여도 Edge 대상이다", () => {
  expect(
    planChanges({
      affected: [],
      edgeConfiguration: true,
      paths: ["supabase/config.toml"],
    })
  ).toMatchObject({ edge: true, validate: true });
});

test("로컬 Supabase 설정만 바뀌면 Edge 대상이 아니다", () => {
  expect(
    planChanges({
      affected: [],
      edgeConfiguration: false,
  ...
  ).toMatchObject({ edge: false, validate: false });
});
